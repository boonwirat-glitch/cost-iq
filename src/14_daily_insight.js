...
// ยังไม่มี: รอบวันจันทร์/วันที่ 1 · TL-Admin · ปุ่มตอนเช็คอิน Echo
// ═══════════════════════════════════════════════════════════════════════════
...
@media (prefers-reduced-motion:reduce){
  #di-sheet,#di-list,#di-sheet.di-in .di-rise,#di-boot{transition:none}
...
function openDailyInsight(){
  ...
  const reveal=()=>{
    ...
    _diFillVisitRow();
    if(d.big&&d.big.celebrate)setTimeout(()=>_diCelebrate(d.big.celebrate),260);
    _diOpening=false;
  };
  ...
  if(body){
    ...
    body.addEventListener('touchmove',e=>{
      if(!tracking||_diListOpen())return;
