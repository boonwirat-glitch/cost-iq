...
let _tlSquadRoles = {};        // ECHO GOAL 2 / Phase S: email(lowercase) -> raw role, roster-sourced
let _tlSquadById  = {};        // ECHO GOAL 2 / Phase S: profiles.id -> {email,role,name}, roster-sourced
...
async function _loadTLSquad() {
  ...
  _tlSquadEmails = [];
  _tlSquadRoles = {};
  _tlSquadById = {}; // ECHO GOAL 2 / Phase S: id -> {email,role,name} — roster-sourced, independent of progress rows
  try {
    ...
    // 2. currentUserProfile (01_core.js `let` — never attached to window, so
    // `window.currentUserProfile` here was always undefined; this step used to
    // silently no-op and fall through to the localStorage/sessionStorage scan)
    if (!tlEmail && typeof currentUserProfile !== 'undefined') tlEmail = (currentUserProfile?.email || '').toLowerCase();
    ...
    if (!squadName) {
      ...
      if ((tlRow?.role || '').toLowerCase() === 'admin') {
        const allRows = await _skFetch(`profiles?select=id,email,role,full_name,kam_name&role=in.(sales,rep,sales_tl,tl,ad,ad_tl,pm,kam)`);
        _tlSquadEmails = (allRows || [])
          .map(r => (r.email || '').toLowerCase())
          .filter(e => e && e !== tlEmail);
        (allRows || []).forEach(r => {
          if (!r.email) return;
          _tlSquadRoles[r.email.toLowerCase()] = r.role;
          if (r.id) _tlSquadById[r.id] = { email: r.email.toLowerCase(), role: r.role, name: r.kam_name || r.full_name || '' };
        });
        _tlSquad = 'ทั้งบริษัท';
    ...
    // ECHO GOAL 2 / Phase S: +id/name so the roster (not just progress rows) can
    // seed the "By Rep" list — a rep with zero progress used to never appear
    const repRows = await _skFetch(`profiles?select=id,email,role,full_name,kam_name&squad=eq.${encodeURIComponent(squadName)}&role=in.(sales,rep,sales_tl,tl,ad,ad_tl,pm)`);
...
async function _loadSkillUsers() {
  ...
  try {
    ...
    // ECHO GOAL 2 / Phase S: +role so _skUserBucket() can resolve each rep's bucket by userId
    const rows = await _skFetch(`profiles?select=id,email,full_name,kam_name,role&id=in.(${uids.join(',')})`);
    (rows || []).forEach(r => {
      if (!r.id) return;
      _skillUsers[r.id] = {
      ...
      if (r.email && r.role) _tlSquadRoles[r.email.toLowerCase()] = r.role;
    });
...
// ── ECHO GOAL 2 / Phase S: role-bucket helpers ──────────────
// _skillDefs itself is NEVER filtered — every helper below filters at the
// point of use, matching the same "keep the full set, filter on read" rule
// used for _rubricCache in 09_conv_intel.js (prevents silent-drop bugs).
function _skBucketForEmail(email) {
  const role = _tlSquadRoles[(email || '').toLowerCase()];
  return (typeof skillRoleBucket === 'function') ? skillRoleBucket(role) : 'kam';
}
// Bucket for a specific rep being viewed by userId (TL contexts). _skillUsers
// is progress-row-sourced (may not have a zero-progress rep yet) — _tlSquadById
// is roster-sourced and covers everyone, so check it first.
function _skUserBucket(userId) {
  const email = _tlSquadById[userId]?.email || (_skillUsers[userId] && _skillUsers[userId].email) || '';
  if (email) return _skBucketForEmail(email);
  return (typeof skillRoleBucket === 'function') ? skillRoleBucket(_skillsRole) : 'kam';
}
// This viewer's own bucket (rep's own home screen)
function _skOwnBucket() {
  return (typeof skillRoleBucket === 'function') ? skillRoleBucket(_skillsRole) : 'kam';
}
function _skDefsForBucket(bucket) {
  return _skillDefs.filter(d => (typeof skillDefMatchesBucket === 'function') ? skillDefMatchesBucket(d, bucket) : true);
}
// Rep's own scoped catalogue for home/module-grid/detail — TL "อ่าน Skills"
// browse mode deliberately sees the FULL catalogue (they coach every role).
function _skOwnScopedDefs() {
  if (_tlBrowseMode) return _skillDefs;
  return _skDefsForBucket(_skOwnBucket());
}
// Real role label for a rep being viewed (TL contexts) — replaces hardcoded "Sales"
function _skUserRoleLabel(userId) {
  const email = _tlSquadById[userId]?.email || (_skillUsers[userId] && _skillUsers[userId].email) || '';
  const role = _tlSquadRoles[email.toLowerCase()];
  if (role && typeof roleLabel === 'function') return roleLabel(role);
  return 'KAM';
}
...
function _skUserName(userId) {
  ...
  // 3. ECHO GOAL 2 / Phase S: roster fallback — a zero-progress rep has no
  // _skillProg row and no _skillUsers entry (that map is progress-seeded), but
  // still has a name via the squad roster query
  const ru = _tlSquadById[userId];
  if (ru) return ru.name || (ru.email ? ru.email.split('@')[0] : userId.slice(0,8));
  // 4. last resort: UUID 8 chars
...
function _updateSkillsNavBadge() {
  ...
  // ECHO GOAL 2 / Phase S: same squad-scoped base as _renderTLShell/_renderTLPendingContent
  // — raw _skillProg can be the WHOLE org's rows when the squad-roster fetch fell back
  // to loading everything, which used to make this badge overcount vs the tab it labels
  const pending = Object.values(window._tlProgFiltered || _skillProg).filter(p => p.state === 'training').length;
...
function _renderTLShell(scr) {
  ...
  const squadName = _tlSquad || (typeof currentUserProfile !== 'undefined' && currentUserProfile && currentUserProfile.squad) || 'Squad';
...
function _renderRepHome() {
  // ECHO GOAL 2 / Phase S: own-scoped catalogue — a rep only sees skills that
  // apply to their own role bucket. TL "อ่าน Skills" browse mode (_tlBrowseMode)
  // is the one exception and sees the full catalogue via the same helper.
  const scopedDefs = _skOwnScopedDefs();
  const scopedIds  = new Set(scopedDefs.map(d => String(d.id)));

  // Count unlocked+mastered — intersect with scope so a stray progress row for
  // a skill outside this rep's current bucket (e.g. role re-scoped later by
  // admin) can never inflate the count past scopedDefs.length
  const unlocked = Object.entries(_skillProg)
    .filter(([sid, p]) => scopedIds.has(sid) && (p.state === 'unlocked' || p.state === 'mastered')).length;
  const total    = scopedDefs.length;

  // Find next skill in training
  const trainingSkill = scopedDefs.find(d => {
  ...
  // Group by module — skip modules with zero skills in scope (closes the 0/0 green-ring case)
  const modules = ['A','B','C','D'].filter(m => scopedDefs.some(d => d.module === m));
  const moduleRows = modules.map(m => {
    const defs = scopedDefs.filter(d => d.module === m);
    ...
    const meta = MODULE_META[m] || { name: m, sub: '', color: '#8E8E93' }; // guard: module outside A-D
...
// ECHO GOAL 2 / Phase S: role badge — only rendered in TL browse mode (own-scoped
// rep view has already filtered to a single bucket, so the badge is redundant there)
const SK_ROLE_LABEL = { kam:'KAM', sales:'Sales', ad:'AD', pm:'PM' };
function _skRoleBadgeHtml(def) {
  if (!_tlBrowseMode) return '';
  const roles = def && def.roles;
  const label = (roles && roles.length) ? roles.map(r => SK_ROLE_LABEL[r] || r).join('/') : 'ALL';
  return `<span class="sk-tl-role-chip" style="margin-left:auto">${label}</span>`;
}

function _renderModuleGrid(module) {
  const meta = MODULE_META[module] || { name: module, sub: '', color: '#8E8E93' }; // guard: module outside A-D
  const defs = _skOwnScopedDefs().filter(d => d.module === module);
...
<div class="sk-card state-${state}" onclick="skillsOpenDetail(${d.id})">
  ...
  <div class="sk-card-body">
    <div class="sk-state-row"><div class="sk-dot"></div><span class="sk-state-label">${label}</span>${_skRoleBadgeHtml(d)}</div>
...
async function _doOpenDetail(skillId) {
  ...
  // ECHO GOAL 2 / Phase S: own-scoped lookup — a deep link to a skill outside
  // this rep's role bucket must not open (defense in depth; the card that would
  // link here is already hidden by _renderRepHome/_renderModuleGrid)
  const def   = _skOwnScopedDefs().find(d => d.id === skillId);
  if (!def) { _renderSkillsScreen(); return; }
...
async function skillsStartTraining(skillId) {
  if (!_skillsUserId) return;
  // ECHO GOAL 2 / Phase S: block starting training on a skill outside this rep's bucket
  const def = _skOwnScopedDefs().find(d => d.id === skillId);
  if (!def) return;
...
function _renderTLOverviewContent() {
  ...
  const userMap = {};
  // ECHO GOAL 2 / Phase S: seed from the roster FIRST — a rep with zero progress
  // rows (brand new, or hasn't self-marked anything yet) used to be silently
  // absent from "By Rep" entirely since userMap was built purely from progress rows
  Object.keys(_tlSquadById).forEach(uid => { userMap[uid] = []; });
  allProg.forEach(p => {
  ...
  if (_ovToggle === 'rep') {
    const repRows = Object.entries(userMap).map(([uid, progs]) => {
      // ECHO GOAL 2 / Phase S: denominator is THIS rep's own role bucket, not
      // the global skill count — a sales rep's ring is out of their ~4 sales
      // skills, not all 15 (which used to make every non-KAM rep's % look low)
      const scopedDefs = _skDefsForBucket(_skUserBucket(uid));
      const scopedIds  = new Set(scopedDefs.map(d => d.id));
      const uCount  = progs.filter(p => scopedIds.has(p.skill_id) && (p.state==='unlocked'||p.state==='mastered')).length;
      const tCount  = progs.filter(p => scopedIds.has(p.skill_id) && p.state==='training').length;
      const uTotal  = scopedDefs.length;
...
<div class="sk-rep-row" onclick="skillsTLOpenRepDetail('${uid}')">
  ...
  <svg class="sk-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
    ...
    const skillRows = ['A','B','C','D'].map(m => {
      ...
      const hd = `<div class="sk-skill-mod-hd">Module ${m} — ${(MODULE_META[m]||{sub:''}).sub}</div>`;
      const rows = defs.map(d => {
        // ECHO GOAL 2 / Phase S: denominator = squad members whose bucket this
        // skill actually applies to, not the whole squad — a sales-only skill
        // in a mostly-KAM squad used to read "0/8 คน" when only 2 were sales.
        // No squad roster loaded (rare fallback) → keep the old unfiltered count.
        const haveRoster = _tlSquadEmails.length > 0;
        const eligibleEmails = haveRoster
          ? _tlSquadEmails.filter(e => (typeof skillDefMatchesBucket === 'function') ? skillDefMatchesBucket(d, _skBucketForEmail(e)) : true)
          : null;
        if (haveRoster && eligibleEmails.length === 0) return ''; // no one in this squad is eligible — skip the row
        const userCount = haveRoster ? eligibleEmails.length : (Object.keys(userMap).length || 1);
        const progs     = allProg.filter(p => p.skill_id === d.id &&
          (!haveRoster || eligibleEmails.includes((p.owner_email||'').toLowerCase())));
...
async function skillsTLOpenEval(userId, skillId) {
  const scr = document.getElementById('scr-skills');
  if (!scr) return;
  // Full-list lookup (not _skOwnScopedDefs) — TL evaluates any rep, any role
  const def   = _skillDefs.find(d => d.id === skillId);
  if (!def) return;
  // ECHO GOAL 2 / Phase S: block evaluating a skill outside the VIEWED rep's
  // bucket (can happen via a stale progress row from before a role change)
  if (typeof skillDefMatchesBucket === 'function' && !skillDefMatchesBucket(def, _skUserBucket(userId))) {
    _skToast(`${def.skill_code.split('_')[0]} ไม่ได้ประเมินสำหรับตำแหน่งของ ${_skUserName(userId)}`);
    return;
  }
  ...
  <div class="sk-tl-zone">
    ...
    <div class="sk-tl-rep-row">
      ...
      <div>
        ...
        <div class="sk-tl-rep-meta">${_skUserRoleLabel(userId)} · self-marked ${p&&p.updated_at?new Date(p.updated_at).toLocaleDateString('th-TH'):''}</div>
      </div>
    </div>
...
// ── TL rep detail ──────────────────────────────────────────
// ECHO GOAL 2 / Phase S: single shared row-list builder — scoped to the VIEWED
// rep's own bucket. Replaces two near-identical copies (skillsTLOpenRepDetail's
// inline renderList + _skRepFilter) that had drifted into subtle duplication.
function _skRepDetailRows(userId, showAll) {
  const progs  = Object.values(_skillProg).filter(p => p.user_id === userId);
  const scoped = _skDefsForBucket(_skUserBucket(userId));
...
function skillsTLOpenRepDetail(userId) {
  ...
  if (!scr) return;
  const progs    = Object.values(_skillProg).filter(p => p.user_id === userId);
  // ECHO GOAL 2 / Phase S: ring denominator is THIS rep's own bucket, not the
  // global skill count, and uCount intersects scope (guards stray progress
  // rows left over from before a role change)
  const scopedDefs = _skDefsForBucket(_skUserBucket(userId));
  const scopedIds  = new Set(scopedDefs.map(d => d.id));
  const uCount   = progs.filter(p => scopedIds.has(p.skill_id) && (p.state==='unlocked'||p.state==='mastered')).length;
  const uTotal   = scopedDefs.length;
...
<div style="overflow-y:auto;flex:1;padding-bottom:84px;">
  <div class="sk-rep-det-hd">
    ...
    <div>
      ...
      <div class="sk-rep-det-meta">${_skUserRoleLabel(userId)} · ${uCount}/${uTotal} unlocked</div>
    </div>
  ...
  <div class="sk-det-skill-list" id="rep-det-list">${_skRepDetailRows(userId, true)}</div>
...
function _skRepFilter(all, userId) {
  ...
  if (!list) return;
  list.innerHTML = _skRepDetailRows(userId, all);
...
<div style="margin-top:16px;padding:0 14px;">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:8px;padding-top:6px;border-top:0.5px solid var(--sk-hairline,#EBEBEB);">
    ${['A','B','C','D'].map(m => { const _mm = MODULE_META[m] || { name: m, color: '#8E8E93' }; return `<span style="display:flex;align-items:center;gap:4px;font-size:var(--text-xs);color:var(--sk-muted,#6A6A6A);font-family:'Noto Sans Thai',sans-serif"><span style="width:7px;height:7px;border-radius:50%;background:${_mm.color};display:inline-block"></span>${_mm.name.replace('The ','')}</span>`; }).join('')}
