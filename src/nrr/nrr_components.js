...
function nrrRenderMovementChart(chartContainerId, tableContainerId, result, opts) {
  ...
  // v_basenote (backlog ข้อ 10): สูตรฐานเต็มๆ อ่านจาก base_audit ที่ engine คาย
  // ออกมาเอง (nrr_logic.js) — บุชขอให้บอกชัดว่าฐานมาจากปัจจัยอะไร หัก churn/
  // waived แล้วหรือยัง · ตัวเลขชุดเดียวกับที่ engine ใช้หารจริง ไม่ใช่คำนวณซ้ำ
  // result จาก nrrComputeRowsPool (PM/Admin/VP) ไม่มี base_audit → ตกไปใช้
  // โน้ตแบบเดิม (เฉพาะ transfer) เหมือนก่อนหน้า ไม่พังอะไร
  var adjNote = '';
  if (result.base_audit && result.base_audit.length) {
    var _bd = nrrBaseDays();
    var _sum = { core: 0, coreN: 0, churn: 0, churnN: 0 };
    var _excl = {};
    result.base_audit.forEach(function (a) {
      var amt = (a.base_norm_30d / 30) * _bd;
      if (a.included) {
        if (a.movement_effective === 'core_nrr_churn') { _sum.churn += amt; _sum.churnN++; }
        else { _sum.core += amt; _sum.coreN++; }
      } else if (a.exclude_reason && a.exclude_reason !== 'base_gmv=0' && a.exclude_reason !== 'duplicate_outlet') {
        var key = a.exclude_reason.split(' ')[0]; // 'new_sales (ไม่ใช่ฐาน NRR)' -> 'new_sales'
        // transfer_in ถูกกันออกจาก baseMap ก็จริง แต่ถูกบวกกลับเต็มจำนวนใน
        // "+ ย้ายเข้า" อยู่แล้ว — โชว์ซ้ำใน "ไม่นับเข้าฐาน" จะอ่านเหมือนเงินหาย
        if (key === 'transfer_in') return;
        if (!_excl[key]) _excl[key] = { amt: 0, n: 0 };
        _excl[key].amt += amt; _excl[key].n++;
      }
    });
