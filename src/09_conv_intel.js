...
const CI = (() => {
  ...
  // v_deadcapture (2026-08-20): จำ mimeType ไว้ต่างหาก เพราะ _teardownRecorder()
  ...
  // v_etareal (2026-08-21): เดิมคิดจากคาบ cron ล้วน ("ทุก 5 นาที" → "ภายใน ~5 นาที")
  // ซึ่งถูกเฉพาะคลิปสั้นที่ถอดจบในคำขอเดียว · คลิปยาวถูกซอยเป็นหลายช่วงและทำได้
  // "ช่วงละหนึ่ง tick" ⇒ คลิป 51 นาที = 9 ช่วง = 45 นาทีเป็นอย่างน้อย ป้ายเดิมจึง
  // สัญญาเวลาที่ทำไม่ได้จริง (ผมคำนวณจากตาราง cron แต่ลืมว่า cron ทำทีละขั้น)
  // ตอนนี้ worker เก็บ listen_state.windows/next ไว้แล้ว จึงนับช่วงที่เหลือได้จริง
  const CRON_STEP_MIN = 5;
  function _reviewEtaText(nowMs, remainingSteps) {
    const steps = (typeof remainingSteps === 'number' && remainingSteps > 0) ? remainingSteps : 1;
    const mins = steps * CRON_STEP_MIN;
    const THAI_OFFSET_MS = 7 * 3600 * 1000;
    const t = new Date((nowMs == null ? Date.now() : nowMs) + THAI_OFFSET_MS); // "เวลาไทย" อ่านผ่าน getUTC*
    const h = t.getUTCHours();
    // ยังอยู่ในเวลาที่ cron เดินอยู่ (9:00-22:55) — บอกเป็นช่วงเวลาที่เหลือจริง
    if (h >= 9 && h < 23) {
      if (steps <= 1) return 'ภายใน ~5 นาที';
      return `ประมาณ ${mins} นาที (เหลือ ${steps} ช่วง)`;
    }
  ...
  function _reviewStepInfo(s) {
    ...
    // v_etareal: นับช่วงที่เหลือจาก listen_state จริงถ้ามี (คลิปยาวถูกซอยหลายช่วง
    // และทำได้ช่วงละหนึ่ง tick) · ถ้าไม่มี = คลิปสั้น/ยังไม่วางแผน → ถือว่าเหลือ 1 ช่วง
    const _ls = s.listen_state;
    const _winTotal = (_ls && Array.isArray(_ls.windows)) ? _ls.windows.length : 0;
    const _winDone = (_ls && typeof _ls.next === 'number') ? _ls.next : 0;
    const _remaining = _winTotal ? Math.max(1, _winTotal - _winDone) : null;
    if (stage === 'uploaded' || stage === 'needs_gemini') {
      return { step: 0, eta: _reviewEtaText(null, _remaining) };
    }
...
`;
  ...
  function _unmount() {
    ...
    //
    // v_uiguard (2026-08-20): ครึ่งหลังของบั๊กเดียวกัน — v269 กันไม่ให้งานเก่าไปปิด
    // "ไมค์" ของงานใหม่แล้ว แต่ยังถอด "หน้าจอ" กับหยุดนาฬิกาของงานใหม่ได้อยู่
    // (pipeline ร้านก่อนหน้าทำเสร็จตอน rep กำลังอัดร้านถัดไป → จอหายทั้งที่ยังอัด
    //  อยู่เบื้องหลัง แล้วไม่มีปุ่มให้กดจบด้วย) · ถ้ามีการอัดที่ยังไหลอยู่จริง
    // แปลว่า sheet บนจอเป็นของการอัดนั้น ไม่ใช่ของงานที่เพิ่งจบ — ห้ามแตะ
    // เส้นที่ปิด sheet อย่างถูกต้องทุกเส้นผ่าน teardown มาก่อนแล้ว (_recorder = null)
    // จึงไม่ติดเงื่อนไขนี้
  ...
  function _minimize() {
    ...
    if (pill) {
      ...
      // v_pillwarn: pill ต้องสะท้อนสถานะ "ตอนนี้" ทุกครั้งที่โผล่ — ถ้าไมค์ยังตายอยู่
      // (เตือนไปแล้วและ rep ยังไม่กด "อัดต่อ") ต้องขึ้นสีเตือนตั้งแต่แรกที่ย่อ ไม่ใช่
      // ขึ้นเฉพาะตอนที่ watchdog ยิงพอดีขณะย่ออยู่
      pill.classList.toggle('fp-warn', !!_recDeadFired);
      _startFloatTimer();
    }
...
<div id="ci-s-result" class="scr">
  ...
  function _initWaveform() {
    ...
    let t = 0;
    // v_uiguard (2026-08-20): คลื่นนี้เคยเป็น Math.sin() ล้วน ไม่ผูกกับเสียงจริงเลย
    // จึงยังกระเพื่อมสวยงามอยู่แม้ไมค์ตายสนิท — เป็น "จอที่โกหก" ชนิดเดียวกับนาฬิกา
    // ที่เดินต่อทั้งที่ไม่ได้อัดอะไร ซึ่งคือเหตุผลที่บั๊กเดิมไม่มีใครจับได้เป็นสัปดาห์
    // ตอนนี้ผูกความสูงกับ "ไบต์เสียงที่เข้ามาจริง" (_recBytes ที่ watchdog นับอยู่แล้ว)
    // ไมค์เงียบ → แท่งแบนราบเอง ไม่ต้องรอ watchdog ครบ 45 วิ ก็เห็นด้วยตาว่าผิดปกติ
    // ⚠ เจตนาไม่ใช้ AnalyserNode/createMediaStreamSource เพราะเคยทำสัญญาณที่อัดได้เพี้ยน
    let _wLastBytes = 0, _wLastAt = Date.now(), _wLevel = 1;
    _waveRef = setInterval(() => {
      t += 0.14;
      const now = Date.now();
      if (now - _wLastAt >= 500) {
        const bps = (_recBytes - _wLastBytes) / ((now - _wLastAt) / 1000);
        _wLastBytes = _recBytes; _wLastAt = now;
        // 6,800 B/วิ = ย่านปกติที่วัดได้จาก production · ขยับเข้าหาค่าใหม่แบบนุ่มๆ
        // ไม่กระตุก และคงพื้นไว้ 0.12 เพื่อให้ยังเห็นว่าแอปไม่ค้าง
        const target = _phase === 'recording' ? Math.max(0, Math.min(1, bps / 6800)) : 1;
        _wLevel += (target - _wLevel) * 0.5;
      }
      const lv = 0.12 + _wLevel * 0.88;
      ...
      bars.forEach((b, i) => {
        ...
        const h = 6 + s * 26 * lv; // min 6px — แอมพลิจูดตามเสียงจริง
        const op = 0.28 + s * 0.52 * lv;
  ...
  // v_uiguard (2026-08-20): ใช้เกณฑ์เดียวกับ _checkRecoverBuffer เพื่อตอบว่า "ตอนนี้
  // มีเสียงที่กู้ได้ค้างอยู่จริงไหม" — ใช้ก่อนล้าง buffer ตอนเริ่มอัดรอบใหม่
  // (ถ้าเคยกด "ทิ้ง" ที่แถบกู้ไปแล้ว buffer จะว่าง ฟังก์ชันนี้คืน 0 จึงไม่ถามซ้ำ)
  async function _idbRecoverable() {
    try {
      const meta = await _idbGetMeta();
      if (!meta) return null;
      if (Date.now() - (meta.started_at || 0) > 24 * 3600 * 1000) return null;
      const chunks = await _idbGetChunks();
      if (!chunks || chunks.length < 5) return null;
      return { secs: chunks.length, name: meta.account_name || '' };  // timeslice 1 วิ = 1 chunk
    } catch (_) { return null; }
  }
  ...
  async function startRecording() {
    ...
    _startRecBusy = true;
    // v_deadcapture (2026-08-20): กันชั้นสุดท้าย — ถ้ายังมี recorder ค้างจากเส้นทาง
    // ที่คาดไม่ถึง ต้องปิดไมค์ให้สนิทก่อนขอ getUserMedia ใหม่เสมอ ไม่งั้นได้ตัวอัด
    // สองตัวแย่งไมค์กันแล้วตัวจริงตายเงียบๆ ตลอดทั้ง visit
    if (_recorder) _teardownRecorder('stale-before-start');
    ...
    try {
      ...
      // v_uiguard (2026-08-20): ของเดิมล้าง buffer ทิ้งเงียบๆ ทุกครั้งที่เริ่มอัด แม้มี
      // เสียงที่ยังไม่ได้วิเคราะห์ค้างอยู่ · แถบ "พบการบันทึกค้าง" ขึ้นให้เห็นก่อนจริง
      // แต่ถ้า rep กดอัดร้านใหม่เลย เสียง visit ก่อนหน้าหายถาวรโดยไม่มีใครรู้ตัว
      // ถามก่อนเสมอ และถ้าไม่ยืนยันก็ไม่เริ่มอัด (ปิดไมค์ที่เพิ่งขอมาให้เรียบร้อย)
      const _stale = await _idbRecoverable();
      if (_stale && !confirm(
            'มีเสียงที่ยังไม่ได้วิเคราะห์ค้างอยู่ ' + _fmt(_stale.secs) + ' นาที' +
            (_stale.name ? '\n(' + _stale.name + ')' : '') +
            '\n\nถ้าเริ่มอัดใหม่ เสียงชุดนั้นจะหายถาวร\nกดยกเลิกเพื่อกลับไปกด "วิเคราะห์ต่อ" ที่แถบด้านบนก่อน')) {
        _teardownRecorder('keep-stale-buffer');   // ยังไม่ start() แต่ไมค์เปิดแล้ว
        _phase = 'idle';
        _startRecBusy = false;
        _toast('ยังไม่เริ่มอัดใหม่ — กด "วิเคราะห์ต่อ" ที่แถบด้านบนก่อน');
        return;
      }
  ...
  function stopRecording() {
    ...
    _durText = _fmt(_secs);
    // v_deadcapture: keepOnStop — นี่คือการหยุดที่ตั้งใจ ต้องปล่อยให้ onstop ยิง
    // _onStop() ต่อไปตามเดิม (teardown จัดการ timer/track/audioCtx/wakelock ให้ครบ)
    _teardownRecorder('user-stop', { keepOnStop: true });
  ...
  function cancel() {
    ...
    const pill = document.getElementById('echo-float-pill');
    if (pill) { pill.classList.remove('visible'); pill.classList.remove('fp-warn'); }
    // Reset CI fullsheet display if minimized
    const sheet = document.getElementById('ci-fullsheet');
    if (sheet) sheet.style.display = '';
    // v_deadcapture: ยกเลิกแล้วห้ามยิง pipeline — teardown ถอด onstop ให้เอง
    // (ของเดิมเรียก _recorder.stop() ทั้งที่ onstop ยังผูกอยู่ แล้วรอดมาได้เพราะ
    //  _chunks ถูกล้างทันก่อน onstop จะยิง — บังเอิญ ไม่ใช่ตั้งใจ)
    _teardownRecorder('cancel');
    _phase = 'idle';
    _applyRecordingTheme(false);
    _chunks = [];
  ...
  // ── v_deadcapture (2026-08-20): ปิดไมค์ให้สนิท ทุกทางออก ───────────────────
  //
  // บั๊กจริงที่ทำให้ 5 จาก 20 การอัดยาวได้ไฟล์แทบว่างเปล่า (25% · วันที่ 19 ส.ค.
  // พัง 4 จาก 9) โดยนาฬิกาบนจอยังนับครบ 40-70 นาทีเหมือนปกติ:
  //   _recorder.stop() เคยมีแค่ใน stopRecording() กับ cancel() เท่านั้น
  //   แต่ open() รีเซ็ต _phase='idle' แล้วเรียก _unmount() ซึ่งไม่เคยสั่งหยุด
  //   recorder หรือ track เลย → ตัวอัดเก่าค้างจับไมค์ไว้เงียบๆ → พอกดอัดใหม่
  //   ด่าน _phase !== 'idle' ก็ผ่านฉลุย เกิด MediaRecorder สองตัวแย่งไมค์ตัวเดียว
  //   (iOS ทำให้ตัวหนึ่งตาย) แถม _chunks = [] ยังล้างเสียงที่อัดไว้ทิ้งอีก
  // หลักฐาน production: KAM เช็คอิน 06:11 (ไม่มีการอัดจบ) แล้วเช็คอินร้านเดิมซ้ำ
  // 06:39 → session ที่สองอัด 72 นาที ได้ไฟล์ 44KB (ปกติต้อง ~5,150-8,910 B/วิ)
  //
  // ตัวนี้คือทางออกเดียวที่ทุกเส้นต้องเรียก · idempotent เรียกซ้ำได้ไม่พัง
  ...
  function _onDeadCapture(reason) {
    ...
    // v_pillwarn (2026-08-21): แถบเตือนข้างบนอยู่ใน #ci-fullsheet ซึ่งถูก display:none
    // ตอนย่อเป็น pill ⇒ ตอนย่อจะไม่เห็นอะไรเลย เหลือแค่การสั่นเป็นสัญญาณเดียว · และ
    // v276 เพิ่งทำให้การย่อเป็นท่ามาตรฐาน (แตะตัวเลขเวลา) = สถานะย่อกลายเป็นสถานะปกติ
    //
    // **ไม่**สั่งขยายจอกลับเอง เพราะจะขัดกับเหตุผลทั้งหมดของ v276 (ไม่ให้ลูกค้าเห็นว่า
    // มี AI ช่วยสรุป) — ทำให้ pill เองเปลี่ยนเป็นสีเตือนแทน: rep เห็นได้ทันทีจากมุมตา
    // แต่ลูกค้าที่ไม่รู้ว่าปกติเป็นสีอะไรก็ยังไม่รู้ว่าเกิดอะไรขึ้น
    try {
      const pill = document.getElementById('echo-float-pill');
      if (pill) pill.classList.add('fp-warn');
    } catch(_) {}
  ...
  function _recDismissDead() {
    ...
    // v_pillwarn: ปิดสีเตือนบน pill ด้วย ไม่ใช่แค่แถบในจอ — ถ้าไมค์ยังไม่ฟื้นจริง
    // watchdog จะยิงใหม่หลังพ้นช่วงพักแล้วทาสีกลับมาเอง
    try {
      const pill = document.getElementById('echo-float-pill');
      if (pill) pill.classList.remove('fp-warn');
    } catch(_) {}
  }
  ...
  function _teardownRecorder(reason, opts) {
    ...
    if (r) {
      try {
        // ปิดเพราะออกจากหน้า (open/_unmount) ห้ามให้ onstop ยิง pipeline ซ้ำ
        // ส่วนการกดหยุดเองต้องปล่อยให้ยิง จึงส่ง keepOnStop:true มา
        if (!keepOnStop) r.onstop = null;
        if (r.state !== 'inactive') r.stop();
      } catch(_) {}
      try {
        r.stream?.getTracks().forEach(t => {
          try { t.onmute = t.onunmute = t.onended = null; } catch(_) {}
          t.stop();
        });
      } catch(_) {}
    }
    ...
    _releaseWakeLock();
    if (r && reason) console.log('[CI rec] teardown:', reason);
  }
  ...
  // v_deadcapture (2026-08-20): ค่าบนคือ "บิตเรตที่สั่ง" ไม่ใช่ขนาดไฟล์จริงต่อวินาที
  // วัดจาก production 20 ไฟล์: ของดีอยู่ที่ 5,150-8,910 B/วิ (มี container overhead
  // ทับอยู่) ค่ากลาง ~6,800 · การใช้ 6,000 คำนวณ "ได้เสียงกี่นาที" ทำให้รายงานเกิน
  // จริงได้ถึง ~49% จึงแยกค่าคงที่ออกมาเฉพาะสำหรับเลขที่โชว์ให้คนอ่าน
  // ส่วน ratio ยังคิดบนค่าเดิม เพราะเกณฑ์ 0.15 จับไฟล์พังได้ครบทุกใบอยู่แล้ว
  const REC_REAL_BYTES_PER_SEC = 6800;
  function _audioLooksTruncated(blobSize, secs) {
    ...
    const gotMins = Math.round(blobSize / REC_REAL_BYTES_PER_SEC / 60);
  ...
  async function _startAsyncPipeline(blob) {
    ...
    try {
      ...
      // ── v_audior2 (2026-08-21): อัปขึ้น R2 ก่อน ถอยไป Supabase ถ้าล้ม ──────────
      //
      // ทำไมย้าย: Supabase คิดค่า "ขาออก" ทุกครั้งที่ worker ดึงไฟล์ไปถอดเสียง
      // (พิสูจน์จาก log จริง: 71 จาก 88 request เป็น cache HIT = ถูกนับเต็มขนาดไฟล์)
      // ทำให้เกินโควตา 327% · R2 ไม่คิดค่าขาออก ⇒ ยอดฝั่ง Supabase เหลือ 0
      // การอัปโหลด "ขาเข้า" ไม่เคยมีค่าใช้จ่ายทั้งสองฝั่ง — ที่แพงคือขาออก
      //
      // ลำดับสำคัญมาก: **เสียงที่อัดมาแล้วหายคือความเสียหายที่กู้ไม่ได้** ห้ามแลกกับ
      // การประหยัดค่า egress ⇒ ถ้า R2 ล้มด้วยเหตุใดก็ตาม ถอยไปอัป Supabase แบบเดิม
      // ทันที (worker อ่านได้ทั้งสองที่อยู่แล้ว จึงถอดเสียงได้เหมือนกัน)
      let _store = 'r2';
      try {
        const _r2res = await fetch(
          `${WORKER_URL}/audio-put?path=${encodeURIComponent(path)}`,
          { method: 'POST', headers: { 'Content-Type': blob.type || 'audio/webm' }, body: blob });
        if (!_r2res.ok) throw new Error('R2 ตอบ ' + _r2res.status + ': ' + (await _r2res.text().catch(() => '')).slice(0, 120));
      } catch (_r2e) {
        console.warn('[CI upload] อัป R2 ไม่สำเร็จ — ถอยไป Supabase:', (_r2e && _r2e.message) || _r2e);
        _store = 'supabase';
        const { error: upErr } = await supa.storage.from('ciq-data')
          .upload(path, blob, { upsert: true, contentType: blob.type || 'audio/webm' });
        if (upErr) throw upErr;   // ล้มทั้งสองที่ = ปล่อยให้ catch ข้างนอกจัดการ (เสียงยังอยู่ใน IndexedDB)
      }
      console.log(`[CI upload] เก็บไฟล์เสียงที่ ${_store}: ${path} (${(blob.size / 1048576).toFixed(1)}MB)`);
      _uploadedOk = true;
      ...
      // v_deadcapture (2026-08-20): ของเดิมเขียนว่า "ไฟล์เสียงไม่สมบูรณ์" ซึ่งชี้ไป
      // ทางไฟล์ถูกตัด/อัปโหลดไม่ครบ — ผิดทาง และทำให้ไล่สาเหตุผิดอยู่หลายรอบ
      // ความจริงคือไมค์ไม่ได้ส่งเสียงเข้ามาเลยตั้งแต่ต้น (ตัวอัดค้าง/ถูก iOS ตัด)
      // เขียนใหม่ให้ตรงกับสิ่งที่เกิดจริง และบอก rep ว่า visit ยังนับให้ตามปกติ
      const _deadNote = _dead
        ? `${nowIso} [client] ไมค์ไม่ได้รับเสียงตลอดการอัด ${Math.round(ctx.secs / 60)} นาที (ได้ข้อมูลเสียงเกือบเป็นศูนย์ ~${_trunc.gotMins} นาที) — visit นับปกติ แต่วิเคราะห์ไม่ได้`
        : null;
      if (rowId) {
      ...
      if (_dead) {
        ...
        // v_deadcapture (2026-08-20): ของเดิมบอก rep ว่า "ครั้งหน้าลองไม่ล็อกจอ" —
        // โยนความผิดให้คนใช้ทั้งที่ต้นเหตุจริงคือ recorder ค้างในโค้ดเรา (ดู
        // _teardownRecorder) · เอาคำแนะนำที่ทำอะไรไม่ได้ออก เหลือแต่ความจริง
        _setStep('บันทึก visit แล้ว', 'แต่ไมค์ไม่ได้รับเสียง — วิเคราะห์ไม่ได้', 100);
        setTimeout(() => {
          _unmount();
          _toast('ไมค์ไม่ได้รับเสียงตลอดการอัด — visit นับปกติ แต่วิเคราะห์ไม่ได้');
        }, 2400);
        return;
      }
  ...
  async function _loadInlineHistory() {
    ...
    try {
      ...
      let q = supa.from('ci_sessions')
        .select('id,owner_email,account_id,account_name,visited_at,duration_secs,skill_scores,customer_intel,next_actions,transcript_summary,tone_signals,tl_reviewed_at,tl_reviewed_by,tl_note,covisit_verified,status,pipeline_stage,listen_state')
        .order('visited_at', { ascending: false })
        .limit(isTL ? 100 : 50);
...
.sd2-note-ro { font-size:13.5px;color:#3D3680;line-height:1.7;padding:11px 13px;background:rgba(83,74,183,.05);border:0.5px solid rgba(83,74,183,.16);border-radius:11px;margin-bottom:4px; }
  ...
  function _renderSessionDetailContent(s) {
    ...
    // v_usability (2026-08-21): เหตุผลข้างบนหมดอายุแล้ว · ตอนนี้ worker คิด
    // transcript_confidence จาก "ความใช้ได้จริง" (ครอบคลุม × ความละเอียด × ความชัด)
    // ด้วยสูตรเดียวกันทั้ง Gemini และ Whisper (ดู _usabilityScore ฝั่ง worker) ⇒ ตัวเลข
    // มีความหมายเท่ากันทั้งสองแหล่งแล้ว ไม่ต้องมีเกณฑ์พิเศษต่อแหล่งอีก
    // ตรวจกับ session จริง 6 ตัวแล้ว 0.6 แยกงานดี/งานพังได้ถูกทุกตัว: ของ Dent
    // 19 ส.ค. (บุชยืนยันว่าแม่น) ได้ 0.73 = ไม่เตือน · ตัวที่ถอดได้ 27% ได้ 0.27 = เตือน
    const _confThreshold = 0.6;
    let confBanner = '';
...
<div style="text-align:center;padding:36px 16px;color:var(--tx3,#AEAEB2);font-family:'Noto Sans Thai',sans-serif">
  ...
  ${s.pipeline_error ? `<div style="margin-top:14px;font-size:var(--text-2xs);color:var(--tx3,#AEAEB2);font-family:'IBM Plex Mono',monospace;word-break:break-all;opacity:.7">${String(s.pipeline_error).slice(0, 160)}</div>` : ''}
    ...
    // v_audiofix: needs_gemini = ไฟล์ดีแต่ใหญ่เกินขีดของตัวถอดปัจจุบัน — ยังไม่ตาย
    // รอเส้นทางใหม่มารับช่วง จึงแสดงเหมือน 'uploaded' (กำลังทำอยู่) ไม่ใช่ล้มเหลว
    if (s.pipeline_stage === 'uploaded' || s.pipeline_stage === 'no_speech' || s.pipeline_stage === 'needs_gemini') {
      const _isProcessing = s.pipeline_stage === 'uploaded' || s.pipeline_stage === 'needs_gemini';
      body.innerHTML = `
<div class="sd2-name">${repName}</div>
<div class="sd2-meta">${acctLabel} · ${date}${dur !== '—' ? ' · ' + dur : ''}</div>
${checkinBar}
...
<div class="sd2-note-editor" id="sd2-note-editor">
  ...
  function _renderInlineHistory(sessions) {
    ...
    function _renderSessionCard(s, opts) {
      ...
      // v_audiofix: needs_gemini นับเป็น "ยังทำอยู่" ด้วย (ไฟล์ดี แต่รอตัวถอดที่รับไหว)
      if (s.pipeline_stage === 'uploaded' || s.pipeline_stage === 'needs_gemini') {
  ...
  function open(accountGuid) {
    ...
    //
    // v_openguard (2026-08-21): แต่ "ไม่ถูกลบ" ไม่พอ — rep ต้องรู้ตัวว่าเพิ่งตัดการอัด
    // ที่กำลังไปได้ดีทิ้ง · เส้นทางหลัก echoOpen() กันไว้แล้ว (เด้งไป echoExpand)
    // แต่ยังมีอีก 2 ทางที่เรียก open() ตรงๆ โดยไม่มีการ์ด: echoHistory() (จุด Echo ใน
    // หน้าพอร์ต — src/06_portview_teamview.js) และ ciOpen() · และการย่อเป็น pill
    // (v276) ปลดล็อกการเลื่อนหน้าจอ ทำให้กดไปโดนทางพวกนั้นง่ายขึ้นมาก
    // วางการ์ดที่ open() เองจึงครอบทุกทางเรียกในครั้งเดียว
    if (_phase === 'recording' && _recorder && _recorder.state === 'recording') {
      // _fmt() คืนรูป m:ss — เขียนต่อท้ายว่า "นาที" ไม่ได้ (11 วินาทีจะอ่านเป็น
      // "0:11 นาที") เป็นบั๊กป้ายเดียวกับที่ _idbRecoverable ทำอยู่ ไม่ทำซ้ำ
      const _ok = confirm('กำลังอัดอยู่ ' + _fmt(_secs) + ' (นาที:วินาที)\n\n' +
        'ออกไปตอนนี้จะหยุดการอัดครั้งนี้ (เสียงที่อัดไว้ยังกู้คืนได้จากหน้า Echo)\n\n' +
        'ต้องการออกไปเลยไหม?');
      if (!_ok) return;   // ไม่ยืนยัน = ไม่แตะอะไรเลย อัดต่อเหมือนไม่มีอะไรเกิดขึ้น
    }
  ...
  (function _initVisibilityGuard() {
    document.addEventListener('visibilitychange', function() {
      ...
      if (_recorder && _recorder.state === 'inactive' && _chunks.length > 0) {
        _durText = _fmt(_secs);
        // v_deadcapture: keepOnStop — ด้านล่างเรียก _onStop() เองอยู่แล้ว และ
        // recorder ตัวนี้ state==='inactive' อยู่แล้ว stop() จึงไม่ยิงซ้ำ
        _teardownRecorder('os-stopped', { keepOnStop: true });
        _phase = 'processing';
        _applyRecordingTheme(false);
        _showScreen('ci-s-proc');
        _setStep('กำลังวิเคราะห์...', 'การบันทึกถูกหยุดโดยระบบ — ใช้เสียงที่มี', 14);
