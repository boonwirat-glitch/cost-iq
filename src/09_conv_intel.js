...
/* ── TOPBAR ── */
#ci-fullsheet .topbar{display:flex;align-items:center;justify-content:space-between;padding:calc(16px + env(safe-area-inset-top,0px)) 24px 10px;background:transparent;}
...
ตอบ JSON เท่านั้น ไม่มี markdown:
{
  ...
  function _renderResult() {
    ...
    // v606: 3 tabs — บทสนทนา / ทักษะ / ลูกค้า
  ...
  // ── v606: "บทสนทนา" panel — รวม overview + transcript เป็น tab เดียว ──────
...
<div class="sd2-tone">
  ...
  <div class="sd2-tcard"><div class="k">Engagement</div><div class="v" style="color:${cEng}">${engTxt}</div><div class="n">${tone.customer_engagement_note||''}</div></div>
    ...
    // v606: แสดง key_moments ครบทุกจุด (ไม่ตัด top 5 อีกต่อไป)
    const allM = (tone?.key_moments||[]).map(m => _kmText(m)).filter(Boolean);
    const momentsHtml = allM.length
      ? `<div class="sd2-lbl">Key Moments · ${allM.length} จุด</div>`
    ...
    return (toneHtml + summaryHtml + momentsHtml)
      || `<div style="padding:24px;text-align:center;font-size:13px;color:#AEAEB2">ไม่มีข้อมูลบทสนทนา</div>`;
  }
