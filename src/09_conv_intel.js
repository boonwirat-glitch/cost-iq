...
<div id="ci-s-result" class="scr">
  ...
  async function _processBlob(blob, _resumeFromSegments) {
    ...
    try {
      ...
      if (!segments) {
        ...
        const transcriptResult = await _callTranscript(blob, transcriptTimeout, _ctx.accountName);
  ...
  async function _callTranscript(audioBlob, timeoutMs, accountName) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      try {
        res = await fetch(`${WORKER_URL}/transcript`, {
          ...
          body: JSON.stringify({ audio_b64: b64audio, mime_type: mimeType, duration_secs: _secs, account_name: accountName || undefined }),
          signal: ctrl.signal,
        });
...
<div style="margin-bottom:8px">${rows}</div>`;
  ...
  function _customerPanel(d) {
    ...
    // ── A2v2.2: restaurant-lens sections (needs / unknowns / progress) ───────
    // Rows analyzed by the async brain carry these; legacy rows simply don't
    // have the fields and every section below collapses to '' — backward safe.
    const NEED_TYPE = {
      product:'สินค้า', price:'ราคา', delivery:'การส่ง', credit:'เครดิต',
      quality:'คุณภาพ', service:'บริการ', other:'อื่นๆ'
    };
    const needs = Array.isArray(d?.needs) ? d.needs : [];
    const needsHtml = needs.length ? `<div class="sd2-lbl">ความต้องการลูกค้า · ${needs.length} ข้อ</div>` +
      needs.map(n => {
        ...
        const srcChip = n.intensity === 'implied'
          ? `<span class="sd2-sstate no" title="${(n.inferred_from||'').replace(/"/g,'&quot;')}">อ่านระหว่างบรรทัด</span>`
          : `<span class="sd2-sstate no">ลูกค้าพูดเอง</span>`;
        const quote = (n.quote && String(n.quote).trim())
          ? `<div class="sd2-sev">&ldquo;${n.quote}&rdquo;</div>` : '';
        const imp = n.implication
          ? `<div style="font-size:13.5px;color:#48484A;line-height:1.65;margin-top:5px">${n.implication}</div>` : '';
        const act = n.suggested_action
          ? `<div style="font-size:var(--text-base);color:#534AB7;line-height:1.65;margin-top:5px;padding-left:10px;border-left:2px solid rgba(83,74,183,.3)">เกมที่ควรเดิน: ${n.suggested_action}</div>` : '';
        return `<div style="padding:12px 0;border-bottom:0.5px solid #ECECF0">
          <div style="display:flex;align-items:flex-start;gap:8px">
            <div style="flex:1;font-size:var(--text-lg);font-weight:var(--fw-semi);color:#1C1C1E;line-height:1.6">${n.need || '-'}</div>
    ...
    const unknownsHtml = unknowns.length ? `<div class="sd2-lbl">สิ่งที่ควรถามครั้งหน้า</div>` +
      unknowns.map((q, i) =>
        `<div class="sd2-next"><span class="num">${String(i+1).padStart(2,'0')}</span><div>${q}</div></div>`
      ).join('') : '';
    ...
    const progHtml = prog.length ? `<div class="sd2-lbl">ความคืบหน้าจากครั้งก่อน</div>` +
      prog.map(p => {
        const v = PROG_V[p.verdict] || [p.verdict || '—', 'dev'];
        return `<div style="padding:10px 0;border-bottom:0.5px solid #ECECF0">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
            <span style="font-size:var(--text-lg);font-weight:var(--fw-semi);color:#1C1C1E">${p.topic || '-'}</span>
            <span class="sd2-sstate ${v[1]}">${v[0]}</span>
          </div>
          <div style="font-size:13.5px;color:#48484A;line-height:1.65;margin-top:4px">ครั้งก่อน: ${p.before || '—'}<br>รอบนี้: ${p.now || '—'}</div>
        </div>`;
      }).join('') : '';
    ...
    const hasNew   = needsHtml || unknownsHtml || progHtml;
    const intelHtml = hasIntel
      ? `<div class="sd2-lbl">ข้อมูลลูกค้า (OCPB)</div>${blocks}`
      : ((nextsHtml || hasNew) ? '' : `<div style="padding:24px;text-align:center;font-size:var(--text-base);color:#AEAEB2">ยังไม่มีข้อมูลลูกค้าจาก session นี้</div>`);
    // A2v2.2 order: needs → unknowns → progress → OCPB → next steps
    return needsHtml + unknownsHtml + progHtml + intelHtml + nextsHtml;
  }
...
.sd2-note-ro { font-size:13.5px;color:#3D3680;line-height:1.7;padding:11px 13px;background:rgba(83,74,183,.05);border:0.5px solid rgba(83,74,183,.16);border-radius:11px;margin-bottom:4px; }
    ...
    try {
      ...
      const { data, error } = await supa.from('ci_sessions')
        .select('id,owner_email,account_id,account_name,visited_at,duration_secs,skill_scores,customer_intel,next_actions,transcript_summary,tone_signals,tl_reviewed_at,tl_reviewed_by,tl_note,covisit_verified,status,pipeline_stage,transcript,summary_data,rep_lat,rep_lng,checked_in_at,transcript_confidence')
        .eq('id', sessionId)
        .single();
