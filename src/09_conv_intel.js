...
`;
  ...
  function _buildSalesPickerInline() {
    return `
      ...
      <input id="ci-sales-name-inline" type="text" placeholder="ค้นหาหรือพิมพ์ชื่อร้าน..." autocomplete="off"
        style="width:100%;padding:13px 16px;border:1.5px solid #FF385C;border-radius:12px;font-size:15px;outline:none;font-family:'Noto Sans Thai',sans-serif;background:#fff;color:#1C1C1E;-webkit-appearance:none;margin-bottom:8px"
        oninput="CI._pickerSearchSalesInline(this.value)"
        onkeydown="if(event.key==='Enter')CI._pickerConfirmSales(this.value)" />
      <div id="ci-pk-list-sales-inline" style="max-height:200px;overflow-y:auto;margin-bottom:8px"></div>
      ...
      <div style="text-align:center;font-size:12px;color:#AEAEB2;padding:4px 0">เลือกจากรายการ หรือพิมพ์ชื่อใหม่แล้วกดปุ่ม</div>`;
  }

  function _pickerSearchSalesInline(q) {
    const list = document.getElementById('ci-pk-list-sales-inline');
    if (!list) return;
    try {
      const src = (window.salesPortviewData && window.salesPortviewData.length)
        ? window.salesPortviewData
      ...
      let filtered;
      if (!qLow) {
        filtered = src.slice().sort((a, b) => (b.gmvToDate || b.gmv || 0) - (a.gmvToDate || a.gmv || 0)).slice(0, 6);
      } else {
        filtered = src.filter(r => (r.name || r.outlet_name || '').toLowerCase().includes(qLow)).slice(0, 8);
      }

      if (!filtered.length) {
        list.innerHTML = '<div style="text-align:center;padding:12px 0;font-size:12px;color:#AEAEB2">'
          + (q ? 'ไม่พบร้านค้า — กดปุ่มเริ่มบันทึกเพื่อใช้ชื่อที่พิมพ์' : '') + '</div>';
        return;
      }

      list.innerHTML = filtered.map(function(r) {
        var name = r.name || r.outlet_name || '-';
        var seg  = r.accountType || r.account_type || r.seg || '';
        var safeN = name.replace(/'/g, '');
        var html = '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:10px;background:rgba(255,56,92,.05);margin-bottom:4px;cursor:pointer"'
          + ' onclick="document.getElementById(\'ci-sales-name-inline\').value=\'' + safeN + '\';CI._pickerConfirmSales(\'' + safeN + '\')">'
          + '<span style="font-size:13px;font-weight:500;color:#1C1C1E">' + name + '</span>'
          + (seg ? '<span style="font-size:10px;font-weight:600;color:#FF385C">' + seg + '</span>' : '')
          + '</div>';
        return html;
      }).join('');
