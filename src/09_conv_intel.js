    ...
    <div style="display:flex;align-items:center;gap:10px">
      <button id="ci-stop-btn" onclick="CI.stopRecording()"
        style="flex:1;padding:15px 0;border:none;border-radius:16px;font-size:15px;font-weight:500;cursor:pointer;
               ...
               background:rgba(255,56,92,.12);color:#FF385C;
               border:0.5px solid rgba(255,56,92,.25);
...
<div id="ci-s-result" class="scr">
  ...
  function _initWaveform() {
    ...
    const phases = Array.from({length: N}, (_, i) => i * 0.72 + Math.random() * 0.5);
    for (let i = 0; i < N; i++) {
    ...
    _waveRef = setInterval(() => {
      ...
      bars.forEach((b, i) => {
        ...
        const s = Math.abs(Math.sin(t + phases[i]));
...
<div style="padding:10px 20px max(20px,calc(env(safe-area-inset-bottom,0px) + 12px));display:flex;gap:8px">
  ...
  function _applyRecordingTheme(isRec) {
    ...
    if (isRec) {
      ...
      if (sb) { sb.style.background='rgba(255,56,92,.18)'; sb.style.color='rgba(255,120,120,.9)'; sb.style.borderColor='rgba(255,56,92,.3)'; }
      // tab bar
      ...
      // stop btn
      const sb = document.getElementById('ci-stop-btn');
      if (sb) { sb.style.background='rgba(255,56,92,.12)'; sb.style.color='#FF385C'; sb.style.borderColor='rgba(255,56,92,.25)'; }
      // tab bar
      const tabRec = document.getElementById('ci-tab-rec');
