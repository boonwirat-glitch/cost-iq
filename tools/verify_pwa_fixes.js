...
// ── v_updatepill: แถบชวนอัปเดต ──────────────────────────────────────────────
// 2026-08-25 ทำเป็นการ์ดที่ตอบข้อลังเล 3 ข้อเป็นชิป + ปุ่ม "ไว้ทีหลัง"
// 2026-08-28 บุชดูของจริงแล้วบอกว่า "ยุ่งยากเกินไป" — ชิปกับปุ่มปฏิเสธแย่ง
//            ความสำคัญกับปุ่มหลัก ⇒ เหลือแถบเดียว ปุ่มเดียว ไม่มีทางเลือกอื่น
//            รอบแรกทาเขียวทั้งแถบจนปุ่มจมหาย ⇒ แบบ A: พื้นเขียวเข้มนิ่งๆ
//            + ปุ่มเขียวสด เป็นของสว่างชิ้นเดียวบนแถบ (บุชเลือกจาก mockup 4 แบบ)
// สิ่งที่ยังต้องจริงเหมือนเดิม: กดแล้วต้องไม่หลุดล็อกอิน ไม่มีข้อมูลหาย
const sh = fs.readFileSync(path.join(__dirname, '..', 'src', 'shell.html'), 'utf8');
const pillFn = (sh.match(/function _showUpdatePill\(\)\{[\s\S]*?\n          \}/) || [''])[0];

check('เหลือข้อความชวนแค่ประโยคเดียว — ข้อที่คนกลัวที่สุดคือมันนานไหม',
  /1-2 วินาที/.test(pillFn) &&
  !/ไม่ต้องล็อกอินใหม่/.test(pillFn) && !/ข้อมูลไม่หาย/.test(pillFn),
  'ชิปหลายอันไปแย่งความสำคัญกับปุ่มหลัก จนคนเลือกทางที่ง่ายที่สุดคือไม่กด');

check('เป็นแถวเดียวจริง — ห้ามตัดคำขึ้นบรรทัดใหม่จนแถบสูงเป็นการ์ด',
  /white-space:nowrap/.test(pillFn) && /width:max-content/.test(pillFn),
  'เคยพลาดมาแล้ว: กล่อง flex หดจนข้อความขึ้นบรรทัดละคำ แถบสูง 113px');

check('มีปุ่มจริงหนึ่งปุ่ม และมีปุ่มเดียว',
  (pillFn.match(/<button/g) || []).length === 1 &&
  /id="sense-update-go"/.test(pillFn),
  'บุชขอปุ่มจริง (2026-08-28) — รอบก่อนทำทั้งแถบเป็นปุ่ม ตาเลยไม่รู้ว่ากดตรงไหน');

check('ปุ่มเป็นของสว่างชิ้นเดียว พื้นแถบต้องนิ่งกว่าปุ่ม',
  /#sense-update-go\{[^}]*linear-gradient\(140deg,#34C759,#00A651\)/.test(pillFn) &&
  /#sense-update-pill\{[\s\S]{0,600}?background:#132A20/.test(pillFn),
  'รอบก่อนทาเขียวทั้งแถบ ปุ่มเลยจมหายไปกับพื้น');
...
check('กดที่ตัวแถบก็ได้ ไม่ใช่ต้องจิ้มปุ่มให้ตรงเป๊ะ',
  /p\.addEventListener\('click',_supRun\)/.test(pillFn));

check('มีตาข่ายถ้าอัปเดตไม่คืบใน 6 วิ — ไม่ปล่อยวงหมุนค้างตลอดกาล',
  /},6000\);/.test(pillFn) && /location\.reload\(\)/.test(pillFn));

check('ไม่มีทางเลือกอื่นนอกจากอัปเดต — ไม่มีปุ่มปฏิเสธ ไม่มีกากบาท ไม่มีการจำว่าเคยปิด',
  !/sense-update-later/.test(pillFn) && !/ไว้ทีหลัง/.test(pillFn) &&
  !/sense-update-x/.test(pillFn) && !/pill_dismissed/.test(pillFn));

check('แถบชวนกดตลอด ไม่ใช่ประกายวูบเดียวแล้วเงียบ (ไม่มีปุ่มปิดแล้ว มันต้องชวนต่อ)',
  /_supShine[\s\S]{0,80}infinite/.test(pillFn));
