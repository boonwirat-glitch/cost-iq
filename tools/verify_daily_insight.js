...

{
  ...
  check('หัวข้อหน้ารายการบอกว่ากี่ร้านน่าห่วง และที่เหลืออีกกี่ร้าน',
    d._diListTitle('overdue', data).s ===
      '1 ร้านน่าห่วง เดือนละ ฿52,200 · อีก 1 ร้านมีของเลยรอบบ้างแต่ยังไม่น่าห่วง',
    'ได้ ' + d._diListTitle('overdue', data).s);
  check('บรรทัดร้านในลิสต์บอกเงินต่อเดือนก่อน ไม่ใช่จำนวนรายการก่อน',
    /เดือนละ ฿52,200/.test(html) && html.indexOf('เดือนละ ฿52,200') < html.indexOf('2 รายการ'));
...

{
  ...
  check('หัวข้อกลุ่มบอกจำนวนร้าน ไม่ใช่ "วันนี้ยังมี"',
    /di-rest-k">ร้านที่น่าเป็นห่วง '/.test(DI) && /'<b>'\+all\.length\+' ร้าน<\/b>'/.test(DI)
    && !/วันนี้ยังมี/.test(DI));
...

{
  ...
  check('หมายเหตุอธิบาย "ซื้อเพิ่มขึ้น" ตรงกับที่คำนวณจริง',
    /ยอดเฉลี่ยต่อวันของเดือนที่แล้วมาคูณ '\+win\.days\+' วัน/.test(DI));
  check('หมายเหตุบอกด้วยว่าตัวเลข + คือส่วนต่าง ไม่ใช่ยอดซื้อ',
    /คือส่วนต่าง ไม่ใช่ยอดซื้อ/.test(DI) && /ไม่ใช่ยอดคาดการณ์สิ้นเดือน/.test(DI));
}
...

{
  ...
  check('ไม่มี "/ด." โผล่ในหน้ารายการ', html.indexOf('/ด.') === -1);
...

{
  ...
  check('แถวบอกหน่วยเป็นคำที่คนพูดกัน ไม่ใช่ "/ด."',
...

{
  ...
  check('หน่วยเขียนที่หัวข้อครั้งเดียว ไม่แปะซ้ำทุกแถว',
    /ยอดต่อเดือนที่หายไป/.test(html) && !/<small>ต่อเดือน<\/small>/.test(html));
  check('แต่ละร้านบอกสัดส่วนที่หายไปของร้านนั้น', /หายไป 45% ของร้าน/.test(html));
  check('สัดส่วนปัดเป็นจำนวนเต็ม ไม่มีทศนิยมบนจอเช้า',
    !/หายไป \d+\.\d+% ของร้าน/.test(html));
...
// ─────────────────────────────────────────────────────────────────────────────
// 24. ฝั่งข่าวดีก็ต้องมีชื่อร้าน · แถวต้องเตี้ยพอให้เห็นหลายร้าน
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n── 24. ฝั่งข่าวดีและความสูงของแถว ──');

{
  const accounts = [], churn = {}, movement = {};
  // 6 ร้านซื้อเพิ่ม (ยอดไล่ลง) · สองร้านแรกมีของที่ไม่เคยซื้อด้วย
  [[300000, 2], [200000, 0], [150000, 3], [90000, 0], [50000, 0], [20000, 0]].forEach(([ahead, nsku], i) => {
    const id = 'A' + (i + 1);
    // expected 100000 ⇒ gmvToDate = 100000 + ahead ⇒ pct = ahead/1000
    accounts.push({ id, name: 'ร้านโต ' + (i + 1), gmvToDate: 100000 + ahead,
      lastGmv: 500000, paceSignal: { expected: 100000, lastGmv: 500000 } });
    if (nsku) movement[id] = { recentMo: 'ก.ค. 2569',
      newSkus: Array.from({ length: nsku }, (_, k) => ({ id: 'n' + k, name: 'ของใหม่ ' + k, gmv: 9000 })) };
  });
  const s = makeSandbox({ accounts, churn, movement });
  const data = s.buildDailyInsight();
  const html = s._diRenderBody(data);

  check('นับร้านที่ซื้อเพิ่มได้ครบ', data.aheadShops === 6, 'ได้ ' + data.aheadShops);
  check('ฝั่งข่าวดีวางชื่อร้านออกมา ไม่ใช่แถวเดียวมียอดรวม',
    /ร้านโต 1/.test(html) && /ร้านโต 2/.test(html));
  check('หัวข้อฝั่งข่าวดีบอกจำนวนร้าน', /ซื้อเพิ่มขึ้น <b>6 ร้าน<\/b>/.test(html));
  check('หัวข้อฝั่งข่าวดีบอกยอดรวมด้วย', /· รวม \+฿/.test(html));
  check('โชว์ตามเพดาน 4 ร้าน', (html.match(/di-shop di-c\d" data-shop="A/g) || []).length === 4);
  check('บอกว่ายังมีอีกกี่ร้าน', /ดูอีก 2 ร้าน · ทั้งหมด 6 ร้าน/.test(html));
  check('ยอดฝั่งข่าวดีเป็นบวกและเป็นสีเขียว', /di-shop-v di-pos">\+฿300,000/.test(html));
  // ตัวเลข +฿ เป็นส่วนต่าง ไม่ใช่ยอดซื้อและไม่ใช่ยอดคาดการณ์สิ้นเดือน
  // ⇒ ต้องมียอดจริงของเดือนนี้อยู่ข้างๆ ให้เทียบ ไม่งั้นอ่านผิดได้ทั้งสองทาง
  check('แถวบอกยอดซื้อจริงของเดือนนี้ ไม่ใช่โชว์แต่ส่วนต่าง',
  ...
  check('ทั้งสองฝั่งใช้โครงแถวเดียวกัน จะได้อ่านคู่กันได้',
    (html.match(/class="di-shop /g) || []).length === 4);

  // ไม่มีร้านซื้อเพิ่มเลย = ไม่ต้องมีบล็อกเปล่า
  const none = makeSandbox({
    accounts: [{ id: 'Z', name: 'z', gmvToDate: 0, lastGmv: 100, paceSignal: { expected: 100, lastGmv: 100 } }],
    churn: { Z: [churnRow('x', 50000, 'gone', 9, 5)] },
  });
  check('ไม่มีร้านซื้อเพิ่ม → ไม่มีบล็อกเปล่าค้างอยู่',
    !/ซื้อเพิ่มขึ้น <b>/.test(none._diRenderBody(none.buildDailyInsight())));
}

{
  // แถวเตี้ยลงจริง ไม่ใช่แค่ตั้งใจ — วัดจากค่าที่เขียนไว้ใน CSS
  const pad = /\.di-shop\{[^}]*padding:(\d+)px 0/.exec(DI);
  check('แถวร้านไม่สูงเกินไป (padding ≤ 10px)', pad && Number(pad[1]) <= 10,
    'ได้ ' + (pad && pad[1]));
