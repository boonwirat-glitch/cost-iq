...
// วัดจริง: JSON กิน ~54 token/ท่อน (8,691 token / 160 ท่อน) โครงห่อล้วนๆ ~25
{
  ...
  // รอบแรกที่ทดลอง: ได้แค่ 105 ท่อน (เฉลี่ย 201 ตัวอักษร) จากคลิป 33 นาที และมี
  // ช่องว่าง 4.2 นาทีที่ไม่มีท่อนเลย — โมเดลรวบเป็นก้อนเพราะไม่มีโครง JSON บังคับ
  check('prompt ประหยัดบังคับความละเอียด (เปลี่ยนคนพูด/ทุก 10-15 วิ)',
    /ทุกครั้งที่เปลี่ยนคนพูด/.test(pc) && /10-15 วินาที/.test(pc));
  check('prompt ประหยัดห้ามรวบหลายประโยคเป็นบรรทัดเดียว',
    /ห้ามรวบหลายประโยคยาวๆ เป็นบรรทัดเดียว/.test(pc));
  check('prompt ประหยัดห้ามข้ามช่วง (กันรูโหว่แบบ 4 นาทีที่เจอ)',
    /ห้ามข้ามช่วงไหน/.test(pc));
}
