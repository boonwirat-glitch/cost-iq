...
## ⚠️ 6. ค้างอยู่ตอนนี้ (เช็คกับ R2 จริงแล้ว 2026-07-17 ~07:00) — ต้อง rerun BigQuery + upload มือ

**อย่าเชื่อ list นี้จาก memory เฉยๆ อีกต่อไป — เคยพลาดมาแล้วรอบนึง (2026-07-17): จดไว้ว่า 5
ไฟล์ค้างทั้งหมด ทั้งที่จริงมีคนรันไปแล้วบางไฟล์เมื่อคืน (ทุกไฟล์ Last-Modified บน R2 = 17 ก.ค.
~00:00-00:24) แต่ผลไม่เท่ากัน —ต้อง `curl -I`/เช็ค column header ของจริงบน R2 ก่อนสรุปทุกครั้ง**,
ไม่ใช่เช็คแค่ git log ว่าโค้ด SQL ถูกแล้ว (โค้ดถูกไม่ได้แปลว่ามีคนรันมันจริง):

| # | SQL (BigQuery) | Upload เป็นชื่อ | สถานะจริง (เช็คจาก R2 แล้ว) |
|---|----|----|----|
| 1 | `sql/Q8E_portview_v3.sql` **(v4)** | `portview.csv` | ❌ **ยังไม่ได้รัน v4** — เช็คแล้วมีแค่ 20 คอลัมน์ ไม่มี `data_asof_date` (คอลัมน์ที่ 21) เมื่อคืนมีคนรัน "ของเดิม" (v3) ซ้ำแทน — ยังต้องรันจริง แก้ปัญหาข้อมูลค้างแบบเคส "หมีปรุง" (ดูข้อ 0b) |
| 2 | `sql/q3_2026_movement_rep_view.sql` | `kam_rep_view.csv` | ⚠️ **เช็คจาก column header ไม่ได้** — บั๊กที่แก้ (`transfer_out_rows` join เดือนผิด) เป็น logic ข้างใน ไม่ใช่ column ใหม่ schema เหมือนเดิมทั้งก่อน/หลังแก้ ไฟล์รีเฟรชเมื่อคืนจริง (Last-Modified 00:18) แต่ไม่ทราบว่าใช้ SQL เวอร์ชันไหนรัน — ต้องให้คนเช็คมือ (เช่นดู KAM ที่โดน transfer ใน ก.ย. มีแถว transfer_out ขึ้นหรือเปล่า) |
| 3 | `sql/q3_2026_movement_vp_view.sql` | `vp_view.csv` | ⚠️ เหมือนข้อ 2 — บั๊ก label เดือน 2-3 เป็น logic ไม่ใช่ column ใหม่ เช็คจาก header ไม่ได้ ไฟล์รีเฟรชเมื่อคืนจริง (00:20) แต่ไม่ทราบเวอร์ชัน SQL ที่รัน |
| 4 | `sql/sales_handover_pipeline.sql` **(v5)** | `sales_handover_pipeline.csv` | ✅ **อัปเดตแล้วจริง** — column ตรงกับ v5 เป๊ะ (`staff_owner`/`first_dollar_date`/`mtd_gmv` ครบ 11 คอลัมน์) — ไม่ต้องรันซ้ำ |
| 5 | `sql/new_skus_portfolio.sql` **(v3)** | `new_skus_portfolio.csv` | ✅ **อัปเดตแล้วจริง** — column ตรงกับ v3 เป๊ะ (4 คอลัมน์: item_id/item_name_th/new_gmv/account_count) — ไม่ต้องรันซ้ำ |

**สรุปงานที่เหลือจริงๆ**: รัน #1 (`Q8E_portview_v3.sql` v4) แน่นอน — ยืนยันชัดจาก column count
ส่วน #2/#3 ให้ตัดสินใจเอง: ถ้าอยากชัวร์ 100% ว่า transfer_out/label ถูกจริง ให้รันซ้ำเลยก็ได้
(รันต่อกันในเซสชัน BigQuery เดียวได้ ไม่ต้องแก้วันที่เอง — auto-derive จาก `CURRENT_DATE`) หรือถ้า
มั่นใจว่าคนที่รันเมื่อคืนใช้ SQL ล่าสุดอยู่แล้วก็ข้ามได้ — ผมยืนยันจาก header อย่างเดียวไม่ได้

**อัปเดต 2026-07-17 บ่าย**: user รัน BigQuery ได้ผลลัพธ์ที่ถูกต้องแล้วจริง (21 คอลัมน์ มี
`data_asof_date`, ผ่านเทส "หมีปรุง" — เห็น GMV/ออเดอร์จริงไม่ใช่ ฿0/churned) แต่ไฟล์ที่ทิ้งไว้ใน
`Downloads/portview.csv` ตอนถามให้เทียบกลับกลายเป็นไฟล์เก่า (20 คอลัมน์ เหมือนของบน R2 เป๊ะ ไม่ใช่
ผลลัพธ์ที่เพิ่งรันได้) — ไฟล์ BigQuery ตัวที่ถูกต้อง (`bquxjob_6efb4fb2_19f6ee5317c.csv`) หายไปจาก
Downloads ระหว่างทาง ไม่ทราบว่าย้าย/ลบ/เปลี่ยนชื่อไปไหน **user ตัดสินใจพักเรื่องนี้ไว้ก่อน
("งั้นเรื่องนี้ช่างมันเถอะ") — ยังไม่ได้อัปโหลดไฟล์ v4 ขึ้น R2 จริง ยังค้างเหมือนเดิม** ถ้าจะกลับมา
ทำต่อ ต้องรัน `Q8E_portview_v3.sql` (v4) ใหม่อีกรอบ (เผื่อไฟล์เดิมหาไม่เจอจริงๆ) แล้วเช็ค 2 จุด
ก่อนอัป: (1) 21 คอลัมน์ มี `data_asof_date`, (2) row count ตรงกับ R2 ปัจจุบัน (667 บัญชี ตอนที่เช็ค
ล่าสุด) — ไม่ต้องรีบ ไม่ใช่ของเร่งด่วน
