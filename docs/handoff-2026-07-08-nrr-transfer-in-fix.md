# Handoff — transfer_in double-count fix (/nrr nrr_v10 + Sense v852) · 2026-07-08
...
## ✅ ฝั่ง Sense — แก้เสร็จแล้วในวันเดียวกัน (Sense v852)

`src/07c_qnrr_view.js` ได้รับเงื่อนไขเดียวกันที่ baseMap build
(`_effectiveMovement(r) !== 'transfer_in'`) → rebuild `dist/sense_v852.html` →
promote เป็น `index.html` + bump `sw.js` CACHE_NAME เป็น `sense-v852`

Verify: extract `_qnrrCompute` จาก 07c ที่แก้แล้วมารันใน Node กับ CSV จริงชุดเดียวกัน —
6/6 scope (Tape/Bookbig/Mild kam · TL Ploy/Name · Org admin) ให้เลข**ตรงกับ /nrr เป๊ะทุกหลัก**
ทั้ง base_gmv และ NRR% → สองแอป in sync 100% แล้ว
