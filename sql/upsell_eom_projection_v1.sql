-- ══════════════════════════════════════════════════════════════════════════
-- Upsell End-of-Month Projection v1 (2026-07-21) — "จบเดือนนี้ P1/P3 จะเป็นเท่าไหร่?"
-- sql/upsell_eom_projection_v1.sql
--
-- ฉายยอดของแต่ละคู่ (สาขา × กลุ่มสินค้า) จาก pace ปัจจุบันไปจนจบเดือนจริง
-- (MTD ÷ วันที่ผ่านมา × จำนวนวันของเดือน — ก.ค. = 31 วัน) แล้วค่อยตัดเกตด้วย
-- กติกา engine เดิมทุกข้อ บนตัวเลขที่ฉายแล้ว:
--   P1: กลุ่มใหม่ (ไม่อยู่ใน baseline เม.ย.-มิ.ย.) และยอดฉายทั้งกลุ่ม ≥ ฿5,000
--   P3: กลุ่มเดิม ยอดฉาย > 2.00× max baseline และส่วนเพิ่มฉาย ≥ ฿8,000
--
-- อ่านคู่กับตัวเลข MTD (คอลัมน์ *_mtd_raw ควรตรงกับ sense_upsell_team.csv
-- เป๊ะ — ใช้เช็คไขว้ว่า simulation ถูก) — ข้อจำกัดเดียวของการฉายแบบนี้:
-- ถือว่าทุกคู่ซื้อสม่ำเสมอทั้งเดือน ร้านที่ซื้อกระจุกต้นเดือนจะถูกฉายสูงไป
-- และคู่ที่ยังไม่เคยซื้อเลยตอนนี้แต่จะเริ่มซื้อปลายเดือน ไม่ถูกนับ (กดต่ำ)
-- สองแรงนี้หักล้างกันบางส่วน — ใช้เป็น "ประมาณการกลาง" ได้
--
-- CTE ทั้งหมดถึง commission_items copy คำต่อคำจาก q3c_upsell_team_summary_v4.sql
-- (v880-fix) — เปลี่ยนเฉพาะส่วน projection ท้ายไฟล์
-- ══════════════════════════════════════════════════════════════════════════
...
-- ── Projection: engine rules re-tested on pace-extended GMV ───────────────
report_bounds AS (
  SELECT em.month_start, em.month_end,
    DATE_DIFF(LEAST(DATE_SUB(CURRENT_DATE('Asia/Bangkok'), INTERVAL 1 DAY), em.month_end), em.month_start, DAY) + 1 AS days_elapsed,
    DATE_DIFF(em.month_end, em.month_start, DAY) + 1 AS days_in_month
  FROM elapsed_months em
  WHERE em.month_no = (SELECT n FROM report_month)
),
proj AS (
  SELECT
    c.kam_email,
    c.is_p1, c.max_bl,
    -- as-of-today (raw MTD) figures — the engine's own current numbers
    c.existing_gmv, c.total_gmv, c.is_p3,
    -- pace-extended to full month
    c.existing_gmv / rb.days_elapsed * rb.days_in_month AS existing_gmv_eom,
    c.total_gmv    / rb.days_elapsed * rb.days_in_month AS total_gmv_eom
...
sim AS (
  SELECT
    kam_email,
    -- P1 as of today (engine-identical) vs projected end-of-month
    CASE WHEN is_p1 = 1 AND total_gmv >= 5000 THEN existing_gmv ELSE 0 END AS p1_mtd_raw,
    CASE WHEN is_p1 = 1 AND total_gmv_eom >= 5000 THEN existing_gmv_eom ELSE 0 END AS p1_eom,
    -- P3 as of today vs projected end-of-month (same 2.00× + min-incr gates)
    CASE WHEN is_p3 = 1 THEN existing_gmv - max_bl ELSE 0 END AS p3_mtd_raw,
    CASE WHEN max_bl > 0 AND is_p1 = 0
          AND existing_gmv_eom > max_bl * 2.00
          AND existing_gmv_eom - max_bl >= v_p3_min_incremental
         THEN existing_gmv_eom - max_bl ELSE 0 END AS p3_eom,
    CASE WHEN is_p1 = 1 AND total_gmv_eom >= 5000 THEN 1 ELSE 0 END AS p1_pairs_eom,
    CASE WHEN max_bl > 0 AND is_p1 = 0
          AND existing_gmv_eom > max_bl * 2.00
          AND existing_gmv_eom - max_bl >= v_p3_min_incremental
         THEN 1 ELSE 0 END AS p3_pairs_eom
  FROM proj
)
...
SELECT * FROM (
  SELECT
    '★ TOTAL' AS kam_email,
    (SELECT days_elapsed  FROM report_bounds) AS days_elapsed,
    (SELECT days_in_month FROM report_bounds) AS days_in_month,
    ROUND(SUM(p1_mtd_raw), 0) AS p1_mtd_raw,
    ROUND(SUM(p1_eom), 0)     AS p1_projected_eom,
    ROUND(SUM(p3_mtd_raw), 0) AS p3_mtd_raw,
    ROUND(SUM(p3_eom), 0)     AS p3_projected_eom,
    ROUND(SUM(p1_eom) + SUM(p3_eom), 0) AS upsell_total_projected_eom,
    SUM(p1_pairs_eom) AS p1_pairs_eom,
    SUM(p3_pairs_eom) AS p3_pairs_eom
  FROM sim
  UNION ALL
  SELECT
    kam_email,
    (SELECT days_elapsed  FROM report_bounds),
    (SELECT days_in_month FROM report_bounds),
    ROUND(SUM(p1_mtd_raw), 0),
    ROUND(SUM(p1_eom), 0),
    ROUND(SUM(p3_mtd_raw), 0),
    ROUND(SUM(p3_eom), 0),
    ROUND(SUM(p1_eom) + SUM(p3_eom), 0),
    SUM(p1_pairs_eom),
    SUM(p3_pairs_eom)
  FROM sim
  GROUP BY kam_email
)
ORDER BY (kam_email = '★ TOTAL') DESC, upsell_total_projected_eom DESC;
