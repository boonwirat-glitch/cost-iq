...
-- ── DIAG: transfer_out 241 outlets — Apr commercial_owner คืออะไร ────────────
SELECT
  COALESCE(ao.commercial_owner, 'no_order_in_apr') AS apr_commercial_owner,
  COUNT(DISTINCT mc.outlet_id) AS outlets,
  ROUND(SUM(mc.base_gmv), 0)   AS base_gmv
FROM mar_cohort mc
