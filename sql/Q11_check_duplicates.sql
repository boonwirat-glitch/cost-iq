...
params AS (
  SELECT
    DATE_TRUNC(CURRENT_DATE(), MONTH)                                        AS perf_start,
    DATE_SUB(DATE_TRUNC(DATE_ADD(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH), INTERVAL 1 DAY)
                                                                             AS perf_end,
    FORMAT_DATE('%Y-%m', CURRENT_DATE())                                     AS perf_label,
    ...
    FORMAT_DATE('%Y-%m', DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH))        AS prev_label,
    DATE_DIFF(
...
-- ══════════════════════════════════════
-- CHECK QUERY: account_id ซ้ำข้าม type
-- 0 rows = OK / มี rows = parser bug
-- ══════════════════════════════════════
q11_check AS (
  SELECT
    account_id,
    ...
    COUNT(*) AS row_count
  FROM (
    SELECT movement_month, 'new_sales' AS movement_type, account_id FROM new_sales_gmv
    UNION ALL
    SELECT movement_month, 'transfer_in' AS movement_type, account_id FROM transfer_in_gmv
    UNION ALL
    SELECT perf_label AS movement_month, 'transfer_out' AS movement_type, account_id FROM transfer_out_filtered
  )
  GROUP BY account_id
  HAVING COUNT(DISTINCT movement_type) > 1
)

SELECT * FROM q11_check
