...
--
-- v7 fixes (vs v6):
--   ADOPT 1: handover/new_sales Path B fallback — outlet ที่ไม่มี new_user_exp_date
--            ใช้ sale_in_quarter CTE ดูว่ามี SALE order ใน Mar/Apr/May/Jun ไหม
--            (adopt มาจาก May2026_KAM_portfolio_reconcile.sql)
--   ADOPT 2: transfer_in/out ระดับ TL/Admin ดูแค่ commercial_owner เปลี่ยนระดับ
--            KAM↔KAM neutralize, KAM→SALE = transfer_out, AD/PM/Admin→KAM = transfer_in
--            (ปัจจุบัน _effectiveMovement() ใน JS handle อยู่แล้ว แต่ SQL ต้องส่ง
--            base_tl_email ถูกต้องเพื่อให้ JS แยก same-squad ได้ — v6 ทำแล้ว)
--   FIX 11: mar_cohort — เปลี่ยน INNER JOIN kam_list เป็น LEFT JOIN
--            outlet ของ KAM ที่ลาออกไม่หายออกจาก core cohort อีก
--
-- v6 fixes (vs v5):
--   FIX 8: apr_labels — เปลี่ยน INNER JOIN kam_list เป็น LEFT JOIN
--           outlet ที่ commercial_owner='KAM' แต่ staff_owner ไม่อยู่ใน roster ปัจจุบัน
--           (เช่น KAM ลาออก email ถูกลบ) จะไม่หายออกไปจาก classification อีก
--           ใช้ kam_list สำหรับ lookup email/tl เท่านั้น — ไม่ใช่ filter
--   FIX 9: May/Jun LEG A — เปลี่ยน JOIN apr_labels ให้ match แค่ outlet_id
--           ไม่ต้อง match period_kam_email — outlet ที่ย้าย KAM ระหว่าง Q
--           จะยังคง label เดิมจาก apr_labels ได้ถูกต้อง
--  FIX 10: May/Jun LEG A — เพิ่ม pre_mar_ownership check ใน branch al.outlet_id IS NULL
--           outlet ที่ไม่มีใน apr_labels: expansion / new_sales (pre-Mar=SALE) / transfer_in
--
-- v5 fixes (vs v4):
...
mar_cohort AS (
  ...
  -- FIX 11: LEFT JOIN — outlet ของ KAM ที่ลาออกไม่หายออกจาก core cohort
  -- commercial_owner='KAM' เพียงพอที่จะนับเป็น Mar cohort
...
-- ── 8a. sale_in_quarter — Path B fallback สำหรับ outlet ที่ไม่มี new_user_exp_date ──
-- adopt มาจาก May2026_KAM_portfolio_reconcile.sql (sale_dates_per_outlet)
-- ดูว่า outlet มี SALE order ในแต่ละเดือนของ Q ไหม
-- ถ้ามี SALE order ใน Mar → handover (รับมาใน Mar)
-- ถ้ามี SALE order ใน Apr/May/Jun → new_sales (รับมาในเดือนนั้น)
-- Priority: Mar > Apr > May > Jun (เดือนที่เร็วที่สุดที่มี SALE order)
sale_in_quarter AS (
  SELECT
    CAST(o.user_id AS STRING) AS outlet_id,
    MAX(CASE WHEN o.delivery_date BETWEEN '2026-03-01' AND '2026-03-31'
              AND UPPER(TRIM(o.commercial_owner)) = 'SALE'
             THEN o.delivery_date END) AS last_sale_in_mar,
    MAX(CASE WHEN o.delivery_date BETWEEN '2026-04-01' AND '2026-04-30'
              AND UPPER(TRIM(o.commercial_owner)) = 'SALE'
             THEN o.delivery_date END) AS last_sale_in_apr,
    MAX(CASE WHEN o.delivery_date BETWEEN '2026-05-01' AND '2026-05-31'
              AND UPPER(TRIM(o.commercial_owner)) = 'SALE'
             THEN o.delivery_date END) AS last_sale_in_may,
    MAX(CASE WHEN o.delivery_date BETWEEN '2026-06-01' AND '2026-06-30'
              AND UPPER(TRIM(o.commercial_owner)) = 'SALE'
             THEN o.delivery_date END) AS last_sale_in_jun
  FROM `freshket-rn.dwh.order` o
  WHERE o.delivery_date BETWEEN '2026-03-01' AND '2026-06-30'
    AND o.account_type IN ('SA','MC','Chain','Unknown')
    AND o.user_id IS NOT NULL
  GROUP BY 1
),
...
apr_labels AS (
  SELECT
    ...
    CASE
      ...
      -- [2] handover Path A: new_user_exp_date = Mar
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) = '2026-03'
        THEN 'handover'

      -- [2b] handover Path B: ไม่มี new_user_exp_date แต่มี SALE order ใน Mar
      WHEN ao.new_user_exp_date IS NULL
        AND siq.last_sale_in_mar IS NOT NULL
        THEN 'handover'

      -- [3] new_sales Path A: new_user_exp_date = Apr/May/Jun
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'

      -- [3b] new_sales Path B: ไม่มี new_user_exp_date แต่มี SALE order ใน Q
      -- Priority Apr > May > Jun
      WHEN ao.new_user_exp_date IS NULL
        AND (siq.last_sale_in_apr IS NOT NULL
          OR siq.last_sale_in_may IS NOT NULL
          OR siq.last_sale_in_jun IS NOT NULL)
        THEN 'new_sales'
  ...
  -- FIX 8: LEFT JOIN — ไม่ filter outlet ออกถ้า KAM ลาออก/ชื่อเปลี่ยน
  -- commercial_owner='KAM' เพียงพอ — kam_list ใช้แค่ lookup email/tl
  LEFT JOIN kam_list k
  ...
  LEFT JOIN sale_in_quarter siq        ON ao.outlet_id = siq.outlet_id
...
may_rows AS (
  ...
  SELECT
    ...
    CASE
      ...
      -- outlet ที่ไม่มีใน apr_labels:
      WHEN al.outlet_id IS NULL AND ofd_may.first_dollar_date >= '2026-04-01' THEN 'expansion'
      -- new_sales Path A
      WHEN al.outlet_id IS NULL
        AND FORMAT_DATE('%Y-%m', mo.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'
      -- new_sales Path B: มี SALE order ใน Q
      WHEN al.outlet_id IS NULL
        AND (siq_may.last_sale_in_apr IS NOT NULL
          OR siq_may.last_sale_in_may IS NOT NULL
          OR siq_may.last_sale_in_jun IS NOT NULL)
        THEN 'new_sales'
  ...
  JOIN kam_list k
    ON mo.commercial_owner = 'KAM'
   AND TRIM(mo.staff_owner) = TRIM(k.kam_name)
  -- FIX 9: match outlet_id เท่านั้น ไม่ต้อง match kam_email
  ...
  LEFT JOIN sale_in_quarter siq_may     ON mo.outlet_id = siq_may.outlet_id
...
jun_rows AS (
  ...
  SELECT
    ...
    CASE
      ...
      -- new_sales Path B: มี SALE order ใน Q
      WHEN al.outlet_id IS NULL
        AND (siq_jun.last_sale_in_apr IS NOT NULL
          OR siq_jun.last_sale_in_may IS NOT NULL
          OR siq_jun.last_sale_in_jun IS NOT NULL)
        THEN 'new_sales'
      WHEN al.outlet_id IS NULL THEN 'transfer_in'
      ELSE al.fixed_label
    END AS movement_type
  ...
  LEFT JOIN sale_in_quarter siq_jun     ON jo.outlet_id = siq_jun.outlet_id
