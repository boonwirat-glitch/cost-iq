...
-- ── DIAG: transfer_in detail — outlet / account / GMV / staff_owner / pre-Mar owner ──
SELECT
  r.period_month,
  r.outlet_id,
  r.account_name,
  ...
  r.period_tl_email,
  r.curr_gmv,
  -- staff_owner ปัจจุบัน (จาก ownership ของเดือนนั้น)
  CASE r.period_month
    WHEN '2026-04' THEN ao.staff_owner
    WHEN '2026-05' THEN mo.staff_owner
    WHEN '2026-06' THEN jo.staff_owner
  END AS current_staff_owner,
  -- commercial_owner ก่อน Mar (มาจากไหน)
  pmo.commercial_owner AS pre_mar_commercial_owner,
  pmo.staff_owner      AS pre_mar_staff_owner
FROM all_rows r
LEFT JOIN apr_ownership ao ON r.outlet_id = ao.outlet_id AND r.period_month = '2026-04'
LEFT JOIN may_ownership mo ON r.outlet_id = mo.outlet_id AND r.period_month = '2026-05'
LEFT JOIN jun_ownership jo ON r.outlet_id = jo.outlet_id AND r.period_month = '2026-06'
