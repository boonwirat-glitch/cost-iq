...
-- DIAG: transfer_in ที่ pre_mar_commercial_owner = NULL
-- ดู from_commercial_owner (เดือนก่อนหน้า) ว่ามาจาก KAM หรือเปล่า
SELECT
  r.period_month,
  r.outlet_id,
  r.account_name,
  r.curr_gmv,
  r.period_tl_email,
  pmo.commercial_owner  AS pre_mar_commercial_owner,
  pmo.staff_owner       AS pre_mar_staff_owner,
  -- owner เดือนก่อนหน้า
  CASE r.period_month
    WHEN '2026-04' THEN mar.commercial_owner
    WHEN '2026-05' THEN apr.commercial_owner
    WHEN '2026-06' THEN may.commercial_owner
  END AS prev_month_commercial_owner,
  CASE r.period_month
    WHEN '2026-04' THEN mar.staff_owner
    WHEN '2026-05' THEN apr.staff_owner
    WHEN '2026-06' THEN may.staff_owner
  END AS prev_month_staff_owner
FROM all_rows r
LEFT JOIN pre_mar_ownership pmo ON r.outlet_id = pmo.outlet_id
-- Mar ownership
LEFT JOIN (
  SELECT CAST(o.user_id AS STRING) AS outlet_id,
         UPPER(TRIM(o.commercial_owner)) AS commercial_owner,
  ...
  WHERE o.delivery_date BETWEEN p.base_start AND p.base_end
    AND o.account_type IN ('SA','MC','Chain','Unknown')
  QUALIFY ROW_NUMBER() OVER (PARTITION BY o.user_id ORDER BY o.delivery_date DESC) = 1
) mar ON r.outlet_id = mar.outlet_id
-- Apr ownership
LEFT JOIN (
  SELECT CAST(o.user_id AS STRING) AS outlet_id,
         UPPER(TRIM(o.commercial_owner)) AS commercial_owner,
  ...
  WHERE o.delivery_date BETWEEN p.apr_start AND p.apr_end
    AND o.account_type IN ('SA','MC','Chain','Unknown')
  QUALIFY ROW_NUMBER() OVER (PARTITION BY o.user_id ORDER BY o.delivery_date DESC) = 1
) apr ON r.outlet_id = apr.outlet_id
-- May ownership
LEFT JOIN (
  SELECT CAST(o.user_id AS STRING) AS outlet_id,
         UPPER(TRIM(o.commercial_owner)) AS commercial_owner,
  ...
  WHERE o.delivery_date BETWEEN p.may_start AND p.may_end
    AND o.account_type IN ('SA','MC','Chain','Unknown')
  QUALIFY ROW_NUMBER() OVER (PARTITION BY o.user_id ORDER BY o.delivery_date DESC) = 1
) may ON r.outlet_id = may.outlet_id
WHERE r.movement_type = 'transfer_in'
  AND r.curr_gmv > 0
  AND pmo.commercial_owner IS NULL
ORDER BY r.period_month, r.curr_gmv DESC
