-- ════════════════════════════════════════════════════════════════════════════
-- STATUS (L-4, corrected 2026-07-06 by Bucci): NOT USED — redundant with rep_view.
-- KAM-level and TL-level views are fully covered by sql/q3_2026_movement_rep_view.sql
-- (KAM grain = latest_staff_owner rows directly; TL grain = client-side rollup of
-- rep_view rows grouped by latest_tl_email in src/07c_qnrr_view.js _qnrrCompute()).
-- This file gives strictly less detail than rep_view for the same scope, so it is
-- NOT maintained or tested going forward. Kept in git history for reference only.
-- (pm_view/admin_view are the exception — those scopes are NOT covered by rep_view
-- and remain active, see the other 2 files in this same commit.)
-- ════════════════════════════════════════════════════════════════════════════

-- ════════════════════════════════════════════════════════════════════════════
-- Q2 2026 Movement — KAM TL View  (v1)
-- sql/q2_2026_movement_tl_view.sql
-- Derived from KAM portfolio view — เพิ่ม base_tl / current_tl (Name vs Ploy)
--
...
WITH
tl_map AS (
  SELECT kam_name, tl_name FROM UNNEST([
    STRUCT('Anusorn (Bookbig) Khamphasuk'         AS kam_name, 'Name' AS tl_name),
    STRUCT('Chaklid (Dent) Nimraor'               AS kam_name, 'Name' AS tl_name),
    STRUCT('Duangruedee (Ning) Bulalom'           AS kam_name, 'Name' AS tl_name),
    STRUCT('Napat (To) Kaikaew'                   AS kam_name, 'Name' AS tl_name),
    STRUCT('Nuttawan (Kwang) Mahaporn'            AS kam_name, 'Name' AS tl_name),
    STRUCT('Ploynitcha (Nitcha) Rujipiromthagoon' AS kam_name, 'Name' AS tl_name),
    STRUCT('Rinlaphat (Mild) Setthasiriwuti'      AS kam_name, 'Name' AS tl_name),
    STRUCT('Guntinun (Monet) Thanoochan'          AS kam_name, 'Ploy' AS tl_name),
    STRUCT('Intuon (Jane) Yanakit'                AS kam_name, 'Ploy' AS tl_name),
    STRUCT('Natchita (Foam) Bunkong'              AS kam_name, 'Ploy' AS tl_name),
    STRUCT('Niracha (Cream) Sangka'               AS kam_name, 'Ploy' AS tl_name),
    STRUCT('Puttipong (Tape) Wanithaweewat'       AS kam_name, 'Ploy' AS tl_name),
...
mar_cohort AS (
  SELECT mo.outlet_id, mo.account_id, mo.account_name, mo.res_name, mo.account_type,
    CASE
      WHEN UPPER(TRIM(mo.commercial_owner)) = 'KAM' THEN mo.commercial_owner
      ELSE 'KAM'
  ...
  WHERE (
    UPPER(TRIM(mo.commercial_owner)) = 'KAM'
    OR (
...
jul_rows AS (
  ...
  SELECT
    ...
    mc.first_dollar_date, mc.first_kam_date, mc.first_dollar_owner,
    oed.new_user_exp_date,
    mc.base_gmv, 0.0,
    CASE
  ...
  WHERE NOT EXISTS (
    ...
    WHERE CAST(o.user_id AS STRING) = mc.outlet_id
      AND DATE(o.delivery_date) BETWEEN p.jul_start AND p.jul_end
      AND UPPER(TRIM(o.commercial_owner)) = 'KAM'
      AND o.account_type NOT IN ('Consumer','Enduser','Exclude','TEST')
  )
...
aug_rows AS (
  ...
  WHERE NOT EXISTS (
    ...
    CROSS JOIN params p
    WHERE CAST(o.user_id AS STRING) = mc.outlet_id
      AND DATE(o.delivery_date) BETWEEN p.aug_start AND p.aug_end
      AND UPPER(TRIM(o.commercial_owner)) = 'KAM'
      AND o.account_type NOT IN ('Consumer','Enduser','Exclude','TEST')
  )
...
sep_rows AS (
  ...
  WHERE NOT EXISTS (
    ...
    CROSS JOIN params p
    WHERE CAST(o.user_id AS STRING) = mc.outlet_id
      AND DATE(o.delivery_date) BETWEEN p.sep_start AND p.sep_end
      AND UPPER(TRIM(o.commercial_owner)) = 'KAM'
      AND o.account_type NOT IN ('Consumer','Enduser','Exclude','TEST')
  )
