...
-- ════════════════════════════════════════════════════════════════════════════
-- PURPOSE: Portfolio-level QNRR สำหรับ TL และ Admin scope
-- LOGIC: เหมือน reconcile SQL (verified) แต่ output ตรงกับ JS parser
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── pre_mar_own: ใช้สำหรับ classify handover/new_sales/comeback ──────────────
pre_mar_own AS (
  SELECT CAST(o.user_id AS STRING) AS outlet_id,
...
-- ── apr_labels: lock classification ตั้งแต่ Apr ─────────────────────────────
apr_labels AS (
  SELECT
    ao.outlet_id, ao.account_id, ao.account_name, ao.account_type,
    ao.staff_owner AS current_staff_owner, ao.new_user_exp_date,
    k_cur.kam_email AS period_kam_email, k_cur.tl_email AS period_tl_email,
    mc.base_kam_email, mc.base_tl_email, mc.base_staff_owner,
    COALESCE(mc.base_gmv, 0) AS base_gmv,
    ofd.first_dollar_date, pmo.commercial_owner AS prev_commercial_owner,

    CASE
      WHEN ofd.first_dollar_date >= '2026-04-01'
        AND pmo.outlet_id IS NULL                                              THEN 'expansion'
...
-- ── may_labels: lock classification สำหรับ outlet ที่เข้า KAM ใน May ─────────
may_labels AS (
  SELECT
    mo.outlet_id,
    COALESCE(k_ml.kam_email, al.period_kam_email) AS period_kam_email,
    COALESCE(k_ml.tl_email, al.period_tl_email)   AS period_tl_email,
    al.base_kam_email, al.base_tl_email,
    COALESCE(al.base_gmv, 0) AS base_gmv,

    CASE
      WHEN ofd_m.first_dollar_date >= '2026-04-01'
      ...
      WHEN FORMAT_DATE('%Y-%m', mo.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        AND (pmo_m.commercial_owner = 'SALE' OR pmo_m.outlet_id IS NULL)      THEN 'new_sales'
      WHEN al.outlet_id IS NOT NULL                                            THEN al.fixed_label
      WHEN pmo_m.commercial_owner = 'KAM'
        AND (mo.new_user_exp_date IS NULL
...
all_rows AS (
  ...
  -- APR LEG A
  SELECT '2026-04' AS period_month, '2026-03' AS base_month,
    CASE
    ...
    al.period_kam_email, NULL AS period_kam_name, al.period_tl_email,
    al.base_kam_email, al.base_tl_email,
    al.account_id, al.account_name, al.account_type, al.outlet_id,
  ...
  SELECT '2026-04', '2026-03',
    CASE WHEN ao.outlet_id IS NULL THEN 'core_nrr_churn' ELSE 'transfer_out' END,
    mc.base_kam_email, NULL, mc.base_tl_email,
    mc.base_kam_email, mc.base_tl_email,
    mc.account_id, mc.account_name, mc.account_type, mc.outlet_id,
    mc.base_gmv, 0
  FROM mar_cohort mc
  ...
  -- MAY LEG A
  SELECT '2026-05', '2026-03',
    CASE
      WHEN al.outlet_id IS NOT NULL AND al.fixed_label = 'core'      AND COALESCE(mg.gmv,0) > 0 THEN 'core_nrr'
      WHEN al.outlet_id IS NOT NULL AND al.fixed_label = 'core'      AND COALESCE(mg.gmv,0) = 0 THEN 'core_nrr_churn'
      ...
      WHEN ml.fixed_label IS NOT NULL THEN ml.fixed_label
      ELSE 'transfer_in'
    END,
    COALESCE(al.period_kam_email, ml.period_kam_email), NULL,
    COALESCE(al.period_tl_email,  ml.period_tl_email),
    COALESCE(al.base_kam_email,   ml.base_kam_email),
    COALESCE(al.base_tl_email,    ml.base_tl_email),
    COALESCE(al.account_id, mo.account_id),
    COALESCE(al.account_name, mo.account_name),
    COALESCE(al.account_type, mo.account_type),
    mo.outlet_id,
    COALESCE(al.base_gmv, ml.base_gmv, 0), COALESCE(mg.gmv,0)
  FROM may_own mo
  LEFT JOIN apr_labels al ON mo.outlet_id = al.outlet_id
  LEFT JOIN may_labels ml ON mo.outlet_id = ml.outlet_id
  ...
  SELECT '2026-06', '2026-03',
    ...
    jo.outlet_id,
    COALESCE(al.base_gmv, ml.base_gmv, 0), COALESCE(jg.gmv,0)
  FROM jun_own jo
