-- ════════════════════════════════════════════════════════════════════════════
-- Q_SALES_ALTS v1: Per-Sales Rep Alternatives Bundle
-- Output: sense_alts_{safeKey}.csv  (upload to R2)
-- Mirrors SQL2_sense_alts.sql but ownership = sales_owner_email
--
-- วิธีใช้: แทน '{SALES_EMAIL}' ด้วย email จริง เช่น malisa.c@freshket.co
--          export → ตั้งชื่อ sense_alts_{safeKey}.csv
--          safeKey = email.toLowerCase().replace(/[^a-z0-9]/g,'_')
--
-- NOTE: ใช้ active outlets เท่านั้น (new_user_exp_date >= today)
-- ════════════════════════════════════════════════════════════════════════════

-- Helper: extract total liters from pack_size string (identical to SQL2)
...
-- Active Sales outlets for this rep
sales_outlets AS (
  SELECT
...
-- Last month's order items for Sales outlets
account_items AS (
  SELECT
    so.account_id,
    item.item_id,
  ...
  QUALIFY ROW_NUMBER() OVER (PARTITION BY so.account_id, item.item_id ORDER BY o.delivery_date DESC) = 1
),

-- Catalog reference: market price per SKU (all accounts, same as SQL2)
catalog AS (
  SELECT
    item.item_id,
    item.item_name_th,
...
SELECT
  -- NOTE: ไม่มี sales_email column ใน output (ไม่ต้องผ่าน splitter)
  -- export แยกทีละ rep → ตั้งชื่อ sense_alts_{safeKey}.csv โดยตรง
  a.account_id,
