-- ══════════════════════════════════════════════════════════════════════════
-- P3 Threshold Sensitivity v1 (2026-07-21) — "ถ้าไม่ตัดที่ 2 เท่า ตัดที่เท่าไหร่ถึงแฟร์?"
-- sql/p3_threshold_sensitivity_v1.sql
--
-- จำลองเกต P3 5 ระดับ (1.10× / 1.25× / 1.50× / 1.75× / 2.00×) บนข้อมูลเดือน
-- ปัจจุบันเดือนเดียว โดยคง "ส่วนเพิ่มขั้นต่ำ ฿8,000" ไว้เท่าเดิมทุกแถว (ถ้าจะลด
-- ตัวนั้นด้วย เป็นอีกคันโยกแยก — v_p3_min_incremental ด้านล่าง แก้เลขเดียวจบ)
--
-- ทุกเกตคำนวณ 2 มุมมองคู่กัน:
--   *_mtd_raw   = ยอดจริง 20 วันชนกับ baseline เต็มเดือน (= สิ่งที่ engine จ่ายวันนี้
--                 ถ้าเปลี่ยนเกต) — กลางเดือนจะต่ำกว่าความจริงเสมอ
--   *_norm30    = ยอดปัจจุบัน normalize เป็น 30 วันก่อนชน baseline — ใกล้เคียง
--                 "เดือนนี้จะจบที่เท่าไหร่" มากกว่า ใช้มุมมองนี้ตัดสินใจเรื่องความแฟร์
--
-- จุดอ้างอิงตอนอ่านผล: มิ.ย. (เดือนเต็ม, เกต 2×) P3 ทีมรวม = 1,210,541 ฿
-- คอมฯ P3 ประเมินที่เรต 3% ของ incremental (เรตเดียวกับที่จ่ายจริงใน มิ.ย.)
--
-- CTE ทั้งหมดถึง commission_items copy คำต่อคำจาก q3c_upsell_team_summary_v4.sql
-- (v880-fix) — เปลี่ยนเฉพาะส่วน simulation ท้ายไฟล์
-- ══════════════════════════════════════════════════════════════════════════
...
-- ── Simulation: same rows, five gates, two measurement bases ──────────────
thresholds AS (
  SELECT t FROM UNNEST([1.10, 1.25, 1.50, 1.75, 2.00]) AS t
),
p3_rows AS (
  -- existing outlets, existing groups only (P3 universe) in the report month
  SELECT
    c.kam_email, c.existing_gmv, c.max_bl,
    -- current GMV normalized to 30 days, so a mid-month run compares
    -- like-for-like against the (already 30d-normalized) baseline
    c.existing_gmv
      / (DATE_DIFF(DATE_SUB(CURRENT_DATE('Asia/Bangkok'), INTERVAL 1 DAY), v_m1_start, DAY) + 1)
      * 30 AS curr_norm30
...
sim AS (
  SELECT
    th.t AS threshold,
    r.kam_email,
    -- basis 1: raw MTD (what the engine would pay as of today)
    CASE WHEN r.existing_gmv > r.max_bl * th.t
          AND r.existing_gmv - r.max_bl >= v_p3_min_incremental
         THEN r.existing_gmv - r.max_bl ELSE 0 END AS incr_mtd_raw,
    CASE WHEN r.existing_gmv > r.max_bl * th.t
          AND r.existing_gmv - r.max_bl >= v_p3_min_incremental
         THEN 1 ELSE 0 END AS pass_mtd_raw,
    -- basis 2: normalized-to-30d (where this month is heading)
    CASE WHEN r.curr_norm30 > r.max_bl * th.t
          AND r.curr_norm30 - r.max_bl >= v_p3_min_incremental
         THEN r.curr_norm30 - r.max_bl ELSE 0 END AS incr_norm30,
    CASE WHEN r.curr_norm30 > r.max_bl * th.t
          AND r.curr_norm30 - r.max_bl >= v_p3_min_incremental
         THEN 1 ELSE 0 END AS pass_norm30
  FROM p3_rows r
  CROSS JOIN thresholds th
)

SELECT * FROM (
  -- ★ TOTAL row per threshold
  SELECT
    threshold,
    '★ TOTAL' AS kam_email,
    SUM(pass_mtd_raw)                  AS groups_passing_mtd,
    ROUND(SUM(incr_mtd_raw), 0)        AS p3_incr_mtd_raw,
    SUM(pass_norm30)                   AS groups_passing_norm30,
    ROUND(SUM(incr_norm30), 0)         AS p3_incr_norm30,
    ROUND(SUM(incr_norm30) * 0.03, 0)  AS est_p3_comm_norm30_at_3pct
  FROM sim
  GROUP BY threshold
  UNION ALL
  -- per-KAM rows
  SELECT
    ...
    kam_email,
    SUM(pass_mtd_raw),
    ROUND(SUM(incr_mtd_raw), 0),
    SUM(pass_norm30),
    ROUND(SUM(incr_norm30), 0),
    ROUND(SUM(incr_norm30) * 0.03, 0)
  FROM sim
  GROUP BY threshold, kam_email
)
ORDER BY threshold, (kam_email = '★ TOTAL') DESC, p3_incr_norm30 DESC;
