...
--   transfer_out   : Mar cohort ออกไปนอก portfolio เดือนนั้น (รายเดือน, curr_gmv=0)
--
-- Reconcile checks:
--   C1: SUM(curr_gmv excl. transfer_out) per portfolio = dwh.order GMV
--   C2: transfer inter symmetry (out count = in count per pair)
--   C3: transfer intra symmetry
--   C4: no duplicate outlet per month (excl. transfer rows)
--   C5: label lock (no outlet has 2+ movement types excl. transfer/core)
--   C6: mar_cohort count + base_gmv ตรง dwh.order
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── 3. First order date ที่ owner เป็น KAM/PM/ADMIN ─────────────────────────
-- ใช้ fallback กรณีไม่มี new_user_exp_date เพื่อแยก handover vs new_sales
...
-- ── 4. GMV per outlet per month — ไม่ filter commercial_owner ────────────────
base_gmv AS (
...
-- ── 6. Pre-period ownership — last order ก่อนแต่ละ period ────────────────────
pre_mar_own AS (
  SELECT
    CAST(o.user_id AS STRING)       AS outlet_id,
    UPPER(TRIM(o.commercial_owner)) AS commercial_owner,
...
-- ── 7. Mar handover outlets — any Mar order ที่มี new_user_exp_date = Mar ─────
...
-- ── 8. Mar cohort — fixed denominator ทั้ง Q ─────────────────────────────────
-- Ownership: last order Mar → commercial_owner IN (KAM,PM,ADMIN)
-- Exclude: new_user_exp_date = Mar (handover)
-- base_gmv: GMV ทุก order ใน Mar ไม่ filter portfolio
mar_cohort AS (
  SELECT
    mo.outlet_id,
    mo.account_id, mo.account_name, mo.res_name, mo.account_type,
    mo.commercial_owner  AS base_portfolio,
    mo.staff_owner       AS base_staff_owner,
    mo.new_user_exp_date,
    ofd.first_dollar_date,
    ofp.first_portfolio_date,
  ...
  LEFT JOIN outlet_first_portfolio_order ofp ON mo.outlet_id = ofp.outlet_id
...
-- ── 9. Classification function (inline CASE) ─────────────────────────────────
-- Priority [1]→[7] ใช้ทุก block
-- [1] expansion  : first_dollar >= Apr + owner ไม่ใช่ SALE
-- [2] handover   : new_user_exp_date = Mar
--                  หรือ fallback: ไม่มี exp_date + first_portfolio_order <= Mar
-- [3] new_sales  : new_user_exp_date ใน Q
--                  หรือ fallback: ไม่มี exp_date + first_portfolio_order ใน Q
-- [4] core       : อยู่ mar_cohort + same portfolio
-- [5] transfer_in: อยู่ mar_cohort + diff portfolio
-- [6] comeback   : ไม่อยู่ mar_cohort + first_dollar < Apr
-- [7] transfer_in: ELSE

-- ── 10. Apr labels — lock classification ──────────────────────────────────────
apr_labels AS (
  SELECT
    ...
    COALESCE(ofp.first_portfolio_date, mc.first_portfolio_date) AS first_portfolio_date,
    pmo.commercial_owner AS pre_mar_portfolio,

    CASE
      -- [1] expansion: สาขาใหม่ใน Q
      ...
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) = '2026-03'
        THEN 'handover'
      -- [2b] handover fallback: ไม่มี exp_date + first portfolio order <= Mar
      WHEN ao.new_user_exp_date IS NULL
        AND COALESCE(ofp.first_portfolio_date, mc.first_portfolio_date) <= '2026-03-31'
        THEN 'handover'
      -- [3] new_sales: exp_date ใน Q
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'
      -- [3b] new_sales fallback: ไม่มี exp_date + first portfolio order ใน Apr
      WHEN ao.new_user_exp_date IS NULL
        AND COALESCE(ofp.first_portfolio_date, mc.first_portfolio_date) BETWEEN '2026-04-01' AND '2026-04-30'
        AND ao.commercial_owner != 'SALE'
        THEN 'new_sales'
      -- [4] core
    ...
    -- cohort_month: เดือนที่ label นี้ถูก lock
    CASE
      WHEN COALESCE(ofd.first_dollar_date, mc.first_dollar_date) >= '2026-04-01'
        AND ao.commercial_owner != 'SALE' THEN '2026-04'
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) = '2026-03' THEN '2026-03'
      WHEN ao.new_user_exp_date IS NULL
        AND COALESCE(ofp.first_portfolio_date, mc.first_portfolio_date) <= '2026-03-31' THEN '2026-03'
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
  ...
  LEFT JOIN outlet_first_portfolio_order ofp ON ao.outlet_id = ofp.outlet_id
  LEFT JOIN pre_mar_own pmo                ON ao.outlet_id = pmo.outlet_id
  WHERE ao.commercial_owner IN ('KAM','PM','ADMIN')
),

-- ── 11. May labels — inherit จาก Apr ถ้า portfolio ไม่เปลี่ยน ─────────────────
may_labels AS (
  SELECT
    ...
    COALESCE(al.first_portfolio_date, ofp.first_portfolio_date) AS first_portfolio_date,
    ...
    CASE
      ...
      WHEN al.outlet_id IS NOT NULL AND al.current_portfolio = mo.commercial_owner
        THEN al.fixed_label
      -- classify ใหม่ถ้า portfolio เปลี่ยน หรือ outlet ใหม่ใน May
      ...
      WHEN mo.new_user_exp_date IS NULL
        AND COALESCE(ofp.first_portfolio_date, al.first_portfolio_date) <= '2026-03-31'
        THEN 'handover'
      WHEN FORMAT_DATE('%Y-%m', mo.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'
      WHEN mo.new_user_exp_date IS NULL
        AND COALESCE(ofp.first_portfolio_date, al.first_portfolio_date) BETWEEN '2026-05-01' AND '2026-05-31'
        AND mo.commercial_owner != 'SALE'
        THEN 'new_sales'
...
apr_rows AS (
  ...
  -- LEG A: outlets ที่มี Apr order ใน portfolio
  SELECT
  ...
  -- LEG A-INTRA: staff เปลี่ยนมือในพอร์ตเดิม — transfer_out row (from base_staff)
  SELECT
  ...
  -- LEG A-INTRA: transfer_in row (from current_staff)
  SELECT
    '2026-04', al.outlet_id, al.account_id, al.account_name, al.res_name, al.account_type,
    al.current_portfolio, al.current_staff_owner,
    al.base_portfolio, al.base_staff_owner,
  ...
  -- ดู last order ของ Apr ว่าไปอยู่ที่ไหน
  SELECT
...
may_rows AS (
  ...
  -- LEG A: outlets ที่มี May order ใน portfolio
  SELECT
  ...
  SELECT
    ...
    CASE
      WHEN mo_any.outlet_id IS NULL THEN NULL
...
jun_rows AS (
  ...
  -- LEG A: outlets ที่มี Jun order ใน portfolio
  SELECT
    '2026-06',
    jo.outlet_id,
    ...
    -- movement_type: inherit จาก Apr → May → classify ใหม่ถ้า Jun-only
    CASE
  ...
  -- LEG B Jun: Mar cohort ที่ไม่มี Jun order ใน portfolio เดิม
  SELECT
    ...
    CASE
      WHEN jo_any.outlet_id IS NULL THEN NULL
...
-- ════════════════════════════════════════════════════════════════════════════
-- RECONCILE CHECKS (รันแยกหลัง run main query)
-- ════════════════════════════════════════════════════════════════════════════
...
-- C3: intra transfer_out = intra transfer_in per staff pair per period
-- C4: outlet ต้องมีแค่ 1 แถวต่อเดือน (excl transfer rows)
--     → SELECT period_month, outlet_id, COUNT(*) FROM final
--       WHERE movement_type NOT IN ('transfer_in','transfer_out')
--       GROUP BY 1,2 HAVING COUNT(*)>1  -- ต้องได้ 0 rows
-- C5: label lock
--     → SELECT outlet_id, COUNT(DISTINCT movement_type) FROM final
--       WHERE movement_type NOT IN ('transfer_in','transfer_out','core_nrr','core_nrr_churn')
--       GROUP BY outlet_id HAVING COUNT(*)>1  -- ต้องได้ 0 rows
-- C6: mar_cohort count + base_gmv ตรง dwh.order โดยตรง
