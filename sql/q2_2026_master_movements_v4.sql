...
--   4. transfer_in  curr_gmv = 0 เสมอ (GMV นับอยู่ใน from_portfolio แล้ว)
--   5. Ownership = last order ของแต่ละเดือน (last order wins)
--   6. ห้ามใช้ dim.user_master
--   7. Intra transfer (staff เปลี่ยนมือในพอร์ตเดิม) ไม่อยู่ใน master
--      → ดูได้จาก current_staff_owner field ใน rep-level view แทน
...
apr_labels AS (
  SELECT
    ...
    CASE
      ...
      -- [5] transfer_in from cohort (inter/external)
...
may_labels AS (
  SELECT
    ...
    CASE
      ...
      -- [2b] handover fallback: pmay=SALE + pmo=SALE/NULL
      ...
      WHEN FORMAT_DATE('%Y-%m', mo.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'
      -- [3b] new_sales fallback: pmay=SALE + pmo≠SALE
      WHEN mo.new_user_exp_date IS NULL
        AND pmay.commercial_owner = 'SALE'
...
-- ── 10. APRIL rows ────────────────────────────────────────────────────────────
-- ไม่มี LEG A-INTRA — intra transfer อยู่ใน rep-level view เท่านั้น
-- transfer_in curr_gmv = 0 เสมอ (GMV นับอยู่ใน from_portfolio แล้ว)
apr_rows AS (
  ...
  SELECT
    ...
    al.base_gmv,
    CASE
      WHEN al.fixed_label = 'transfer_in' THEN 0.0
      ELSE COALESCE(ag.gmv, 0)
    END AS curr_gmv,
    CASE
...
may_rows AS (
  ...
  SELECT '2026-05',
    ...
    ml.base_gmv,
    CASE
      WHEN ml.fixed_label = 'transfer_in' THEN 0.0
      ELSE COALESCE(mg.gmv, 0)
    END AS curr_gmv,
    CASE
...
jun_rows AS (
  ...
  SELECT
    ...
    CASE
      WHEN fixed_mv = 'transfer_in' THEN 0.0
      ELSE COALESCE(jg.gmv, 0)
    END AS curr_gmv,

    fixed_mv AS movement_type,
  ...
  FROM (
    SELECT jo.*,
      CASE
        WHEN al.outlet_id IS NOT NULL AND al.current_portfolio = jo.commercial_owner THEN
          CASE WHEN al.fixed_label='core' AND COALESCE(jg2.gmv,0)>0 THEN 'core_nrr'
               WHEN al.fixed_label='core' AND COALESCE(jg2.gmv,0)=0 THEN 'core_nrr_churn'
               ELSE al.fixed_label END
        WHEN ml.outlet_id IS NOT NULL AND ml.current_portfolio = jo.commercial_owner THEN
          CASE WHEN ml.fixed_label='core' AND COALESCE(jg2.gmv,0)>0 THEN 'core_nrr'
               WHEN ml.fixed_label='core' AND COALESCE(jg2.gmv,0)=0 THEN 'core_nrr_churn'
               ELSE ml.fixed_label END
        WHEN COALESCE(ofd2.first_dollar_date) >= '2026-04-01'
          AND jo.commercial_owner != 'SALE' THEN 'expansion'
        WHEN FORMAT_DATE('%Y-%m', jo.new_user_exp_date) = '2026-03' THEN 'handover'
        WHEN jo.new_user_exp_date IS NULL
          AND pjun2.commercial_owner = 'SALE'
          AND (pmo2.commercial_owner = 'SALE' OR pmo2.outlet_id IS NULL) THEN 'handover'
        WHEN FORMAT_DATE('%Y-%m', jo.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
          THEN 'new_sales'
        WHEN jo.new_user_exp_date IS NULL
          AND pjun2.commercial_owner = 'SALE'
          AND pmo2.commercial_owner IS NOT NULL
          AND pmo2.commercial_owner != 'SALE' THEN 'new_sales'
        WHEN mc2.outlet_id IS NOT NULL AND mc2.base_portfolio = jo.commercial_owner
          THEN CASE WHEN COALESCE(jg2.gmv,0)>0 THEN 'core_nrr' ELSE 'core_nrr_churn' END
        WHEN mc2.outlet_id IS NOT NULL AND mc2.base_portfolio != jo.commercial_owner
          THEN 'transfer_in'
        WHEN mc2.outlet_id IS NULL
          AND COALESCE(ofd2.first_dollar_date) < '2026-04-01'
    ...
    LEFT JOIN mar_cohort mc2          ON jo.outlet_id = mc2.outlet_id
    LEFT JOIN outlet_first_dollar ofd2 ON jo.outlet_id = ofd2.outlet_id
    LEFT JOIN pre_mar_own pmo2        ON jo.outlet_id = pmo2.outlet_id
    LEFT JOIN pre_jun_own pjun2       ON jo.outlet_id = pjun2.outlet_id
    LEFT JOIN jun_gmv jg2             ON jo.outlet_id = jg2.outlet_id
...
-- C2: COUNT(transfer_out inter from=A to=B) = COUNT(transfer_in inter from=A to=B)
-- C3: ไม่มี intra transfer ใน master (ย้ายมาไว้ rep-level view แทน)
