...
--   6. Intra/Inter transfer ไม่อยู่ใน LEG A — outlet ที่โอนระหว่าง portfolio
--      ยังถูก classify เป็น core (ใน current_portfolio) + LEG B generate
--      transfer_out row ใน from_portfolio เพื่อ adjust base
--   7. VP view: WHERE movement_type != 'transfer_out' OR transfer_scope = 'external'
--   8. Handover/new_sales fallback: pre_period=SALE + แยกด้วย pre_mar
--      pre_period=SALE + pre_mar=SALE/NULL → handover
--      pre_period=SALE + pre_mar≠SALE      → new_sales
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── 8. Apr labels ─────────────────────────────────────────────────────────────
-- KEY CHANGE: outlet ที่โอนระหว่าง portfolio (mar_cohort + diff portfolio)
-- → classify เป็น 'core' ใน current_portfolio (ไม่ใช่ transfer_in)
-- → LEG B จะ generate transfer_out row ใน base_portfolio แทน
apr_labels AS (
  SELECT
    ...
    CASE
      ...
      -- [2b] handover fallback: pre_apr=SALE + pre_mar=SALE/NULL
      ...
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
        THEN 'new_sales'
      -- [4] core: mar_cohort อยู่ใน portfolio ไหนก็ตาม (รวม inter transfer)
      ...
      WHEN mc.outlet_id IS NULL
        ...
        THEN 'comeback'
      -- [6] transfer_in: ไม่ใช่ cohort ไม่ใช่ expansion/comeback (outlet ใหม่จาก SALE)
      ELSE 'transfer_in'
    END AS fixed_label,

    -- transfer metadata สำหรับ TL/Rep view
    ...
    CASE
      ...
      WHEN ao.new_user_exp_date IS NULL
        AND papr.commercial_owner = 'SALE'
        AND (pmo.commercial_owner = 'SALE' OR pmo.outlet_id IS NULL) THEN '2026-03'
      WHEN FORMAT_DATE('%Y-%m', ao.new_user_exp_date) IN ('2026-04','2026-05','2026-06')
  ...
  SELECT
    ...
    CASE
      ...
      WHEN mo.new_user_exp_date IS NULL
        ...
        THEN 'new_sales'
      -- [4] core: mar_cohort อยู่ใน portfolio ไหนก็ตาม
      WHEN mc.outlet_id IS NOT NULL THEN 'core'
      -- [5] comeback
      WHEN mc.outlet_id IS NULL
    ...
    COALESCE(al.cohort_month,
      CASE
        ...
        WHEN mo.new_user_exp_date IS NULL
          AND pmay.commercial_owner = 'SALE'
          AND pmo.commercial_owner IS NOT NULL
          AND pmo.commercial_owner != 'SALE' THEN '2026-05'
        ELSE NULL
      END
    ) AS cohort_month
...
apr_rows AS (
  ...
  -- outlet ที่โอนระหว่าง portfolio → core ใน current_portfolio, curr_gmv จริง
  SELECT
  ...
  -- LEG B: Mar cohort ที่ไม่มี Apr order ใน portfolio ไหนเลย → core_nrr_churn
  --        Mar cohort ที่ออกไป SALE → transfer_out external
  --        (inter transfer ถูก handle ใน LEG A แล้ว ไม่ต้อง generate ที่นี่)
  SELECT '2026-04',
    ...
    CASE
      ...
      WHEN ao_any.commercial_owner NOT IN ('KAM','PM','ADMIN') THEN 'transfer_out'
      ELSE NULL  -- inter transfer: outlet ถูก classify ใน LEG A แล้ว ไม่ต้อง generate
    END AS movement_type,
    CASE WHEN ao_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN mc.base_portfolio ELSE NULL END AS from_portfolio,
    CASE WHEN ao_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN ao_any.commercial_owner ELSE NULL END AS to_portfolio,
    CASE WHEN ao_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN 'external' ELSE NULL END AS transfer_scope,
...
may_rows AS (
  -- LEG A
  SELECT '2026-05',
    ml.outlet_id, ml.account_id, ml.account_name, ml.res_name, ml.account_type,
  ...
  SELECT '2026-05',
    ...
    mc.base_gmv, 0.0,
    CASE
      WHEN mo_any.outlet_id IS NULL THEN 'core_nrr_churn'
      WHEN mo_any.commercial_owner NOT IN ('KAM','PM','ADMIN') THEN 'transfer_out'
      ELSE NULL
    END,
    CASE WHEN mo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN mc.base_portfolio ELSE NULL END,
    CASE WHEN mo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN mo_any.commercial_owner ELSE NULL END,
    CASE WHEN mo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN 'external' ELSE NULL END,
    CAST(NULL AS STRING), CAST(NULL AS STRING)
  FROM mar_cohort mc
...
jun_rows AS (
  ...
  SELECT
    ...
    COALESCE(al.cohort_month, ml.cohort_month,
      CASE
        ...
        WHEN jo.new_user_exp_date IS NULL
          ...
          AND pmo.commercial_owner IS NOT NULL
          AND pmo.commercial_owner != 'SALE' THEN '2026-06'
        ELSE NULL
      END
    ) AS cohort_month,
    ...
    CASE
      ...
      -- Jun-only
      WHEN COALESCE(ofd.first_dollar_date) >= '2026-04-01'
        AND jo.commercial_owner != 'SALE' THEN 'expansion'
      WHEN FORMAT_DATE('%Y-%m', jo.new_user_exp_date) = '2026-03' THEN 'handover'
      WHEN jo.new_user_exp_date IS NULL
        AND pjun.commercial_owner = 'SALE'
        AND (pmo.commercial_owner = 'SALE' OR pmo.outlet_id IS NULL) THEN 'handover'
    ...
    COALESCE(al.from_portfolio, ml.from_portfolio) AS from_portfolio,
    COALESCE(al.to_portfolio, ml.to_portfolio)     AS to_portfolio,
    COALESCE(al.transfer_scope, ml.transfer_scope) AS transfer_scope,
    CAST(NULL AS STRING) AS from_staff_owner,
    CAST(NULL AS STRING) AS to_staff_owner
  ...
  SELECT '2026-06',
    ...
    mc.base_gmv, 0.0,
    CASE
      WHEN jo_any.outlet_id IS NULL THEN 'core_nrr_churn'
      WHEN jo_any.commercial_owner NOT IN ('KAM','PM','ADMIN') THEN 'transfer_out'
      ELSE NULL
    END,
    CASE WHEN jo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN mc.base_portfolio ELSE NULL END,
    CASE WHEN jo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN jo_any.commercial_owner ELSE NULL END,
    CASE WHEN jo_any.commercial_owner NOT IN ('KAM','PM','ADMIN')
      THEN 'external' ELSE NULL END,
    CAST(NULL AS STRING), CAST(NULL AS STRING)
  FROM mar_cohort mc
...
-- ════════════════════════════════════════════════════════════════════════════
-- VP VIEW QUERY:
--   WHERE movement_type NOT IN ('transfer_in')
--   AND NOT (movement_type = 'transfer_out' AND transfer_scope = 'inter')
--
-- RECONCILE CHECKS:
-- C1: SUM(curr_gmv excl transfer_out) per portfolio per month = dwh.order GMV
-- C2: transfer_out external only ใน VP view
-- C3: ไม่มี intra/inter transfer ใน VP view
-- C4: outlet 1 แถวต่อเดือน (excl transfer) = 0 duplicates
-- C5: label lock per outlet ตลอด Q
-- C6: mar_cohort ตรง dwh.order
