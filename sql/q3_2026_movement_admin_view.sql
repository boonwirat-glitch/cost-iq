-- ════════════════════════════════════════════════════════════════════════════
-- Q2 2026 Movement — ADMIN Portfolio View  (v1)
-- sql/q2_2026_movement_admin_view.sql
--
-- Scope: ADMIN portfolio เท่านั้น (commercial_owner = 'ADMIN')
--
-- ต่างจาก VP view:
--   [1] mar_cohort  : last Mar order = 'KAM' เท่านั้น
--   [2] curr_gmv    : filter commercial_owner = 'ADMIN' เท่านั้น
--   [3] LEG A       : WHERE commercial_owner = 'ADMIN'
...
--                     หรือ first_admin_date ใน Q (fallback)
...
--   external = ออกไป SALE
--
-- curr_gmv = order ที่ commercial_owner = 'ADMIN' เท่านั้น
...
outlet_first_dollar AS (
  SELECT
    ...
    MIN(CASE WHEN UPPER(TRIM(o.commercial_owner)) = 'ADMIN'
             THEN DATE(o.delivery_date) END) AS first_admin_date,
    ARRAY_AGG(
...
outlet_prev_owner AS (
  ...
  JOIN outlet_first_dollar ofd
   ...
   AND DATE(o.delivery_date) < ofd.first_admin_date
  WHERE o.user_id IS NOT NULL
    AND o.account_type NOT IN ('Consumer','Enduser','Exclude','TEST')
...
jul_gmv AS (
  ...
  WHERE o.delivery_date BETWEEN p.jul_start AND p.jul_end
    AND o.gmv_ex_vat > 0
    AND o.account_type NOT IN ('Consumer','Enduser','Exclude','TEST')
    AND UPPER(TRIM(o.commercial_owner)) = 'ADMIN'
  GROUP BY 1
),
aug_gmv AS (
  SELECT CAST(o.user_id AS STRING) AS outlet_id, ROUND(SUM(o.gmv_ex_vat),0) AS gmv
...
-- KAM mar_cohort: Mar last = 'KAM' หรือ SALE spot + first_admin_date < Apr
mar_cohort AS (
  SELECT mo.outlet_id, mo.account_id, mo.account_name, mo.res_name, mo.account_type,
    CASE
      WHEN UPPER(TRIM(mo.commercial_owner)) = 'ADMIN' THEN mo.commercial_owner
      ELSE 'ADMIN'
  ...
  WHERE (
    UPPER(TRIM(mo.commercial_owner)) = 'ADMIN'
    OR (
      UPPER(TRIM(mo.commercial_owner)) = 'SALE'
      AND ofd.first_admin_date IS NOT NULL
      AND ofd.first_admin_date < '2026-07-01'
    )
  )
...
-- kam_pm_mar_cohort: outlets ที่ Mar last owner = PM หรือ ADMIN
-- ใช้ detect transfer_in ใน KAM view (outlet ย้ายมาจาก PM/ADMIN ใน Q)
kam_pm_mar_cohort AS (
  ...
  WHERE (
    mo.commercial_owner IN ('KAM','PM')
    OR (
      UPPER(TRIM(mo.commercial_owner)) = 'SALE'
      AND ofd.first_admin_date IS NOT NULL
      AND ofd.first_admin_date < '2026-07-01'
      AND UPPER(TRIM(ofd.first_dollar_owner)) IN ('KAM','PM')
    )
  )
...
jul_rows AS (
  ...
  SELECT
    ...
    CASE
      ...
      WHEN FORMAT_DATE('%Y-%m', oed.new_user_exp_date) IN ('2026-07','2026-08','2026-09')
      ...
      WHEN ofd.first_dollar_date < '2026-07-01'
        ...
        AND (oed.new_user_exp_date IS NULL
             OR FORMAT_DATE('%Y-%m', oed.new_user_exp_date)
                NOT IN ('2026-06','2026-07','2026-08','2026-09')
    ...
    CASE
      ...
      WHEN ofd.first_admin_date IS NOT NULL
           THEN FORMAT_DATE('%Y-%m', ofd.first_admin_date)
      ELSE NULL
    END AS cohort_month,
  ...
  LEFT JOIN kam_pm_mar_cohort pamc ON ao.outlet_id = pamc.outlet_id
  WHERE UPPER(TRIM(ao.commercial_owner)) = 'ADMIN'
...
aug_rows AS (
  ...
  LEFT JOIN base_gmv bg              ON mo.outlet_id = bg.outlet_id
  LEFT JOIN kam_pm_mar_cohort pamc ON mo.outlet_id = pamc.outlet_id
  WHERE UPPER(TRIM(mo.commercial_owner)) = 'ADMIN'
  ...
  SELECT
    ...
    mc.base_gmv, 0.0,
    CASE
      WHEN mo_port.commercial_owner IN ('KAM','PM') THEN 'transfer_out'
      WHEN mo_sale.outlet_id IS NOT NULL              THEN 'transfer_out'
      ELSE 'core_nrr'
    END,
    '2026-03',
    CASE
      WHEN mo_port.commercial_owner IN ('KAM','PM') THEN 'inter'
      WHEN mo_sale.outlet_id IS NOT NULL              THEN 'external'
      ELSE NULL
    END
  FROM mar_cohort mc
  LEFT JOIN outlet_exp_date oed  ON mc.outlet_id = oed.outlet_id
  LEFT JOIN aug_own mo_admin  ON mc.outlet_id = mo_admin.outlet_id
    AND mo_admin.commercial_owner = 'ADMIN'
  LEFT JOIN aug_own mo_port ON mc.outlet_id = mo_port.outlet_id
    AND mo_port.commercial_owner IN ('KAM','PM')
  LEFT JOIN aug_own mo_sale ON mc.outlet_id = mo_sale.outlet_id
    AND mo_sale.commercial_owner = 'SALE'
  WHERE mo_admin.outlet_id IS NULL
),
...
sep_rows AS (
  ...
  LEFT JOIN kam_pm_mar_cohort pamc ON jo.outlet_id = pamc.outlet_id
  WHERE UPPER(TRIM(jo.commercial_owner)) = 'ADMIN'
  ...
  SELECT
    ...
    mc.base_gmv, 0.0,
    CASE
      WHEN jo_port.commercial_owner IN ('KAM','PM') THEN 'transfer_out'
      WHEN jo_sale.outlet_id IS NOT NULL              THEN 'transfer_out'
      ELSE 'core_nrr'
    END,
    '2026-03',
    CASE
      WHEN jo_port.commercial_owner IN ('KAM','PM') THEN 'inter'
      WHEN jo_sale.outlet_id IS NOT NULL              THEN 'external'
      ELSE NULL
    END
  FROM mar_cohort mc
  LEFT JOIN outlet_exp_date oed  ON mc.outlet_id = oed.outlet_id
  LEFT JOIN sep_own jo_admin  ON mc.outlet_id = jo_admin.outlet_id
    AND jo_admin.commercial_owner = 'ADMIN'
  LEFT JOIN sep_own jo_port ON mc.outlet_id = jo_port.outlet_id
    AND jo_port.commercial_owner IN ('KAM','PM')
  LEFT JOIN sep_own jo_sale ON mc.outlet_id = jo_sale.outlet_id
    AND jo_sale.commercial_owner = 'SALE'
  WHERE jo_admin.outlet_id IS NULL
),
