...
-- Q8E_portview_v3.sql) — ก่อนหน้านี้ไฟล์นี้ไม่มี 4 คนนี้เลย และ filter
-- commercial_owner='KAM' ตายตัว ทำให้ outlet ของ PM/AD ไม่เข้า upsell เลย
kam_list AS (
...
nrr_core_outlets AS (
  ...
  JOIN kam_list k_m ON m.commercial_owner = k_m.expected_owner AND TRIM(m.staff_owner) = TRIM(k_m.kam_name)  -- 2026-07-24: was ='KAM' (Q8E pattern)
  JOIN kam_list k_a ON a.commercial_owner = k_a.expected_owner AND TRIM(a.staff_owner) = TRIM(k_a.kam_name)  -- 2026-07-24: was ='KAM'
